...
export async function GET(request, { params }) {
    ...
    try {
        ...
        // Construir WHERE con visibilidad por defecto cuando no existe config en productos_catalogo
        let whereClause = `
            WHERE p.activo = TRUE
            AND p.empresa_id = ?
            AND COALESCE(pc.activo, TRUE) = TRUE
            AND COALESCE(pc.visible_catalogo, TRUE) = TRUE
        `
        ...
        if (categoria) {
            ...
            paramsWhere.push(parseInt(categoria))
        }

        // Filtro por búsqueda
        if (busqueda) {
            whereClause += ` AND (p.nombre LIKE ? OR p.descripcion LIKE ? OR p.sku LIKE ?)`
            const busquedaLike = `%${busqueda}%`
            paramsWhere.push(busquedaLike, busquedaLike, busquedaLike)
        }
        ...
        const query = `
            SELECT 
                ...
                CASE 
                    WHEN COALESCE(pc.stock_visible, TRUE) = TRUE THEN p.stock
                    ELSE NULL
                END as stock,
                p.stock_minimo,
                p.activo,
            ...
            ${fromClause}
            ${whereClause}
            ORDER BY COALESCE(pc.destacado, FALSE) DESC, COALESCE(pc.orden_visual, 999999) ASC, p.nombre ASC
            LIMIT ? OFFSET ?
        `

        const paramsQuery = [empresaId, ...paramsWhere, limite, offset]
