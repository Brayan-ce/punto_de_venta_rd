...
export async function obtenerDatosPlataforma() {
    ...
    try {
        ...
        if (config.length > 0) {
            return {
                success: true,
                nombre_plataforma: config[0].nombre_plataforma || 'IsiWeek',
                logo_url: config[0].logo_url || null
            }
        }
        return { success: true, nombre_plataforma: 'IsiWeek', logo_url: null }
    } catch (error) {
        console.error('Error al obtener datos plataforma:', error)
        if (connection) connection.release()
        return { success: false, nombre_plataforma: 'IsiWeek', logo_url: null }
    }
}
