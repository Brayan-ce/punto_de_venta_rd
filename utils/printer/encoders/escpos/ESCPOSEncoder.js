...
export class ESCPOSEncoder {
    ...
    static encodeWithLibrary(ticketData, EscPosEncoder) {
        ...
        // HEADER
        const dirEmpresa = ticketData.empresa.direccion
        const dirValida = dirEmpresa && String(dirEmpresa).trim().toLowerCase() !== 'por definir'
        ...
        encoder
            ...
            .bold(false)
        if (dirValida) {
            encoder.text(dirValida).newline()
        }
        encoder
        ...
        encoder
            ...
            .newline()
            .size('large')
            .text(this.formatMoney(ticketData.total))
            .size('small')
            .bold(false)
            .newline()
            .align('left')
    ...
    static encodeBasic(ticketData) {
        ...
        commands.push(`${ESC}a1${ESC}E1`);
        commands.push('TOTAL\n');
        commands.push(`${this.formatMoney(ticketData.total)}\n`);
        commands.push(`${ESC}E0${ESC}a0`);
        commands.push('-'.repeat(32) + '\n');
