...
// Evita que el mismo par sin conversión llene los logs con cientos de líneas iguales
const _conversionesAdvertidas = new Set()
...
export async function obtenerFactorConversionGrafo(
    ...
    try {
        ...
        if (!camino) {
            const _key = `${unidadOrigenId}-${unidadDestinoId}`
            if (!_conversionesAdvertidas.has(_key)) {
                _conversionesAdvertidas.add(_key)
