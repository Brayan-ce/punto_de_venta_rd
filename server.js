const { createServer } = require('http')
const { parse } = require('url')
const next = require('next')
const fs = require('fs')
const path = require('path')
const cron = require('node-cron')
const mysql = require('mysql2/promise')

// IMPORTANTE: En producción, asegúrate de configurar NODE_ENV=production
// Ejemplo: NODE_ENV=production node server.js
// O en PM2: pm2 start server.js --env production
const dev = process.env.NODE_ENV !== 'production'
const hostname = process.env.HOSTNAME || 'localhost'
const port = process.env.PORT || 3000

// Advertencia si está en modo desarrollo en un entorno que parece producción
if (dev && process.env.PORT && process.env.PORT !== '3000') {
  console.warn('⚠️  ADVERTENCIA: NODE_ENV no está configurado como "production"')
  console.warn('⚠️  El indicador de desarrollo de Next.js puede aparecer')
  console.warn('⚠️  Configura NODE_ENV=production antes de ejecutar en producción')
}

// Configuración de logging
const LOG_FILE = path.join(__dirname, '.cursor', 'debug.log')
const LOG_ENDPOINT = 'http://127.0.0.1:7242/ingest/105a53c3-dc11-4849-aa2d-b5ff204596b0'

// Función helper para logging
function logDebug(data) {
  const logEntry = JSON.stringify({
    ...data,
    timestamp: Date.now(),
    sessionId: 'debug-session',
  }) + '\n'
  
  // Intentar enviar a endpoint HTTP (solo en desarrollo local)
  if (dev) {
    fetch(LOG_ENDPOINT, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    }).catch(() => {})
  }
  
  // También escribir a archivo (útil en producción)
  try {
    const logDir = path.dirname(LOG_FILE)
    if (!fs.existsSync(logDir)) {
      fs.mkdirSync(logDir, { recursive: true })
    }
    fs.appendFileSync(LOG_FILE, logEntry, 'utf8')
  } catch (err) {
    // Silenciar errores de logging
  }
}

const app = next({ dev, hostname, port })
const handle = app.getRequestHandler()

// ─── CRON: generar anuncios recurrentes cada día a las 00:05 ─────────────────
cron.schedule('5 0 * * *', async () => {
  const hoy = new Date()
  const diaHoy = hoy.getDate()
  let conn
  try {
    conn = await mysql.createConnection({
      host:     process.env.DB_HOST     || 'localhost',
      port:     process.env.DB_PORT     || 3306,
      user:     process.env.DB_USER,
      password: process.env.DB_PASSWORD,
      database: process.env.DB_NAME
    })

    // Obtener plantillas mensuales que aplican hoy
    const [plantillas] = await conn.execute(
      `SELECT * FROM announcements
       WHERE recurrence = 'monthly' AND day_of_month = ? AND activo = 1`,
      [diaHoy]
    )

    for (const p of plantillas) {
      // Evitar duplicados: verificar que no se haya creado uno hoy
      const inicioHoy = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate())
      const finHoy    = new Date(inicioHoy.getTime() + 86400000)

      const [existe] = await conn.execute(
        `SELECT id FROM announcements
         WHERE title = ? AND recurrence IS NULL
           AND created_at >= ? AND created_at < ?
         LIMIT 1`,
        [p.title, inicioHoy, finHoy]
      )

      if (existe.length > 0) continue  // Ya creado hoy

      const [res] = await conn.execute(
        `INSERT INTO announcements
         (title, message, type, target_type, is_mandatory, scheduled_at)
         VALUES (?, ?, ?, ?, ?, NOW())`,
        [p.title, p.message, p.type, p.target_type, p.is_mandatory]
      )

      // Copiar targets si aplica
      if (p.target_type === 'specific') {
        const [targets] = await conn.execute(
          `SELECT empresa_id, user_id FROM announcement_targets WHERE announcement_id = ?`,
          [p.id]
        )
        for (const t of targets) {
          await conn.execute(
            `INSERT INTO announcement_targets (announcement_id, empresa_id, user_id) VALUES (?, ?, ?)`,
            [res.insertId, t.empresa_id, t.user_id]
          )
        }
      }

      console.log(`[CRON] Anuncio recurrente creado: "${p.title}" (día ${diaHoy})`)
    }

    await conn.end()
  } catch (err) {
    console.error('[CRON] Error al procesar anuncios recurrentes:', err.message)
    if (conn) await conn.end().catch(() => {})
  }
})
// ─────────────────────────────────────────────────────────────────────────────

// ─── CRON: crear pagos pendientes de suscripciones vencidas cada día ──────────
cron.schedule('10 0 * * *', async () => {
  let conn
  try {
    conn = await mysql.createConnection({
      host: process.env.DB_HOST || 'localhost',
      port: process.env.DB_PORT || 3306,
      user: process.env.DB_USER,
      password: process.env.DB_PASSWORD,
      database: process.env.DB_NAME
    })
    const [suscripciones] = await conn.execute(
      `SELECT * FROM suscripciones_plataforma WHERE activo = 1 AND proxima_fecha <= CURDATE()`
    )
    for (const suscripcion of suscripciones) {
      const periodo = suscripcion.frecuencia === 'daily'
        ? new Date().toISOString().slice(0, 10)
        : new Date().toISOString().slice(0, 7)
      const referencia = `SUSCRIPCION-${suscripcion.id}-${periodo}`
      const [pendientes] = await conn.execute(
        `SELECT id FROM pagos_plataforma WHERE empresa_id = ? AND cliente_email = ? AND concepto = 'Suscripción IsiWeek' AND estado = 'pending' LIMIT 1`,
        [suscripcion.empresa_id, suscripcion.cliente_email]
      )
      if (pendientes.length) continue
      const [resultado] = await conn.execute(
        `INSERT INTO pagos_plataforma (empresa_id, cliente_nombre, cliente_email, negocio_nombre, concepto, moneda, monto, descuento, impuestos, metodo_pago, referencia, fecha_pago, creado_por)
         VALUES (?, ?, ?, ?, 'Suscripción IsiWeek', ?, ?, 0, 0, 'transferencia', ?, NOW(), ?)`,
        [suscripcion.empresa_id, suscripcion.cliente_nombre, suscripcion.cliente_email, suscripcion.negocio_nombre, suscripcion.moneda, suscripcion.monto, referencia, suscripcion.creado_por]
      )
      await conn.execute(
        `INSERT INTO pagos_plataforma_auditoria (pago_id, admin_user_id, accion, estado_nuevo, motivo) VALUES (?, ?, 'CREATE_RECURRING_SUBSCRIPTION_PAYMENT', 'pending', ?)`,
        [resultado.insertId, suscripcion.creado_por, suscripcion.frecuencia]
      )
      await conn.execute(
        `UPDATE suscripciones_plataforma SET proxima_fecha = DATE_ADD(CURDATE(), INTERVAL ${suscripcion.frecuencia === 'daily' ? '1 DAY' : '1 MONTH'}) WHERE id = ?`,
        [suscripcion.id]
      )
    }
  } catch (err) {
    console.error('[CRON] Error al generar pagos de suscripciones:', err.message)
  } finally {
    if (conn) await conn.end().catch(() => {})
  }
})
// ─────────────────────────────────────────────────────────────────────────────

app.prepare().then(() => {
  createServer(async (req, res) => {
    try {
      const parsedUrl = parse(req.url, true)
      const { pathname } = parsedUrl
      
      // Configurar límite de body size para rutas de importación (20MB)
      // Esto previene el error 413 (Request Entity Too Large)
      if (pathname && pathname.includes('/api/productos/importar')) {
        // Aumentar el límite de tamaño del body para esta ruta específica
        req.setTimeout(300000) // 5 minutos timeout
      }

      // #region agent log
      // Logging de requests a chunks para debugging
      if (pathname && pathname.startsWith('/_next/static/chunks/') && pathname.endsWith('.js')) {
        const chunkName = pathname.split('/').pop()
        logDebug({
          location: 'server.js:45',
          message: 'Chunk request received',
          data: {
            pathname,
            chunkName,
            method: req.method,
            headers: {
              host: req.headers.host,
              'user-agent': req.headers['user-agent']?.substring(0, 50),
            },
          },
          runId: 'chunk-debug',
          hypothesisId: 'A',
        })
      }
      // #endregion agent log

      await handle(req, res, parsedUrl)
      
      // #region agent log
      // Logging después de manejar el request (para ver el status code)
      if (pathname && pathname.startsWith('/_next/static/chunks/') && pathname.endsWith('.js')) {
        logDebug({
          location: 'server.js:75',
          message: 'Chunk request handled',
          data: {
            pathname,
            statusCode: res.statusCode,
          },
          runId: 'chunk-debug',
          hypothesisId: 'A',
        })
      }
      // #endregion agent log
    } catch (err) {
      console.error('Error occurred handling', req.url, err)
      
      // #region agent log
      logDebug({
        location: 'server.js:88',
        message: 'Server error handling request',
        data: {
          url: req.url,
          error: err.message,
          stack: err.stack?.substring(0, 200),
        },
        runId: 'error-debug',
        hypothesisId: 'C',
      })
      // #endregion agent log
      
      res.statusCode = 500
      res.end('Internal Server Error')
    }
  }).listen(port, (err) => {
    if (err) throw err
    console.log(`> Ready on http://${hostname}:${port}`)
  })
})

