...
export default function CatalogoPublico() {
    ...
    const abrirProducto = async (producto) => {
        setProductoSeleccionado(producto)
        setImagenSeleccionada(0)
        try {
            const respuesta = await fetch(`/api/catalogo/${slug}/producto/${producto.id}`)
            const resultado = await respuesta.json()
            if (resultado.success) setProductoSeleccionado(resultado.producto)
        } catch (error) {
                ...
                <div className={estilos.modalOverlay} onClick={() => setProductoSeleccionado(null)}>
                    <div className={estilos.modalProducto} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className={estilos.modalProductoImagen}>
                            {([productoSeleccionado.imagen_url, ...(productoSeleccionado.imagenes_catalogo || [])].filter(Boolean))[imagenSeleccionada] ? (
                                <img src={([productoSeleccionado.imagen_url, ...(productoSeleccionado.imagenes_catalogo || [])].filter(Boolean))[imagenSeleccionada]} alt={productoSeleccionado.nombre} />
                            ) : (
                                <div className={estilos.imagenPlaceholder}>
                                    <ion-icon name="image-outline"></ion-icon>
                                    <span>Sin imagen</span>
                        ...
                        {productoSeleccionado.imagenes_catalogo?.length > 0 && (
                            <div className={estilos.miniaturasCatalogo}>
                                {[productoSeleccionado.imagen_url, ...productoSeleccionado.imagenes_catalogo].filter(Boolean).map((imagen, indice) => (
                                    <button key={imagen} type="button" className={imagenSeleccionada === indice ? estilos.miniaturaActiva : ''} onClick={() => setImagenSeleccionada(indice)}><img src={imagen} alt={`${productoSeleccionado.nombre} ${indice + 1}`} /></button>
                                ))}
                            </div>
                        )}
