...
import { obtenerPerfilGoogle, registrarUsuario, registrarUsuarioGoogle } from './servidor'
...
export default function Registro() {
    ...
    useEffect(() => {
        ...
        if (scriptExistente) {
            if (window.google?.accounts?.id) cargarGoogle()
            else scriptExistente.addEventListener('load', cargarGoogle, { once: true })
            return () => scriptExistente.removeEventListener('load', cargarGoogle)
        }
        ...
        script.async = true
        script.defer = true
        script.addEventListener('load', cargarGoogle, { once: true })
        document.head.appendChild(script)
        return () => script.removeEventListener('load', cargarGoogle)
    }, [])

    const manejarGoogleCredential = async ({ credential }) => {
        if (!credential) {
        ...
        try {
            const resultado = await obtenerPerfilGoogle(credential)
            if (!resultado.success) {
                setError(resultado.mensaje || 'No se pudo validar la cuenta de Google')
                return
            }
            ...
            setFormData(prev => ({
                ...prev,
                nombre: resultado.perfil.nombre || prev.nombre,
                email: resultado.perfil.email || prev.email,
                telefono: resultado.perfil.telefono || prev.telefono
            }))
        } catch (error) {
            console.error('Error al validar cuenta de Google:', error)
    ...
    const manejarSubmit = async (e) => {
        ...
        try {
            const resultado = googleCredential
                ? await registrarUsuarioGoogle(formData, googleCredential)
        ...
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            <div className={`${estilos.caja} ${estilos[tema]}`}>
                ...
                <form onSubmit={manejarSubmit} className={estilos.formulario}>
                    ...
                    <div className={estilos.googleRegistro}>
                        <button
                            type="button"
                            className={estilos.botonGoogle}
                            disabled={cargando || cargandoGoogle || !googleDisponible}
                            onClick={() => window.google?.accounts.id.prompt()}
                        >
                            {cargandoGoogle ? (
                                <ion-icon name="hourglass-outline" className={estilos.iconoCargando}></ion-icon>
                            ) : (
                                <FcGoogle className={estilos.iconoGoogle} aria-hidden="true" />
                            )}
                            <span>{googleCredential ? 'Cuenta de Google vinculada' : 'Crear cuenta con Google'}</span>
                        </button>
                        <div className={estilos.divisorGoogle}><span>o completa el registro manualmente</span></div>
                    </div>
