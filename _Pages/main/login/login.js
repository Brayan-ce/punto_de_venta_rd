    ...
    const manejarGoogleCredential = async ({ credential }) => {
        ...
        try {
            ...
            if (!resultado.success) {
                setError(resultado.mensaje || 'No se pudo iniciar sesión con Google')
                return
            }
            ...
            if (resultado.tipo === 'superadmin') router.push('/superadmin')
            else if (resultado.tipo === 'financiamiento') router.push('/financiamiento/dashboard')
            else if (resultado.tipo === 'sucursales') router.push('/sucursales')
            else if (systemMode === 'OBRAS') router.push('/admin/manejo-simple')
            else router.push(resultado.tipo === 'vendedor' ? '/vendedor' : '/admin')
        } catch (error) {
            ...
            case 'texto':
                return (
                    <div key={item.id} className={estilos.guiaItem}>
                        {item.titulo && <h3 className={estilos.guiaItemTitulo}>{item.titulo}</h3>}
                        <div
                            className={estilos.guiaItemTexto}
            ...
            case 'archivo':
                return (
                    <div key={item.id} className={estilos.guiaItem}>
                        ...
                        <a
                            href={item.contenido}
                            download={item.nombre_archivo || item.titulo}
                            className={estilos.pdfLink}
                        >
                            <ion-icon name="download-outline" />
                            <span>Descargar {item.nombre_archivo || item.titulo}</span>
                        </a>
                    </div>
                )
