...
export default function UsuariosSuperAdmin() {
    ...
    const abrirModalCrear = () => {
        ...
        const emp = empresas.find(em => String(em.id) === String(empresaSeleccionada))
        const modo = emp?.es_obras ? 'OBRAS' : 'POS'
        setFormData({
            ...
            email: '',
            password: '',
            tipo: emp?.es_sucursales ? 'sucursales' : 'vendedor',
            system_mode: modo,
            permite_impresion: true
        })
        setErroresForm({})
    ...
    const abrirModalEditar = (usuario) => {
        ...
        const emp = empresas.find(em => String(em.id) === String(usuario.empresa_id))
        const modo = emp?.es_obras ? 'OBRAS' : 'POS'
        setFormData({
            ...
            tipo: usuario.tipo,
            system_mode: modo,
            permite_impresion: usuario.permite_impresion !== false
        })
        setErroresForm({})
        setMostrarModal(true)
    }
    ...
    const manejarCambioInput = (e) => {
        ...
        if (name === 'empresa_id') {
            const emp = empresas.find(em => String(em.id) === String(value))
            const modo = emp?.es_obras ? 'OBRAS' : 'POS'
            setFormData(prev => {
                const tipoActual = prev.tipo
                const tipoAjustado = (!emp?.es_sucursales && tipoActual === 'sucursales') ? 'vendedor' : tipoActual
                return { ...prev, empresa_id: value, system_mode: modo, tipo: tipoAjustado }
            })
        } else {
    ...
    const getModoVisualUsuario = (usuario) => {
        if (usuario?.tipo === 'sucursales') return 'SUCURSALES'
        return usuario?.system_mode || 'POS'
    }

    const getModoVisualFormulario = () => {
        if (formData.tipo === 'sucursales') return 'SUCURSALES'
        return formData.system_mode === 'OBRAS' ? 'OBRAS' : 'POS'
    }
    ...
    const getTipoBadge = (tipo) => {
        const tipos = {
            ...
            sucursales: { texto: 'Sucursales', color: 'sucursales' },
        }
                            ...
                            <div className={estilos.tablaBody}>
                                {usuariosFiltrados.map((usuario) => (
                                    <div key={usuario.id} className={`${estilos.fila} ${estilos[tema]}`}>
                                        ...
                                        <div className={estilos.columna}>
                                            <span className={`${estilos.badgeModo} ${getModoVisualUsuario(usuario) === 'OBRAS' ? estilos.modoObras : (getModoVisualUsuario(usuario) === 'SUCURSALES' ? estilos.modoSucursales : estilos.modoPos)}`}>
                                                {getModoVisualUsuario(usuario)}
                                            </span>
                                        </div>
                                        <div className={estilos.columna}>
                        ...
                        <form onSubmit={manejarSubmit} className={estilos.modalBody}>
                            ...
                            <div className={estilos.filaForm}>
                                ...
                                <div className={estilos.grupoInput}>
                                    ...
                                    >
                                        ...
                                        {(() => {
                                            const emp = empresas.find(em => String(em.id) === String(formData.empresa_id))
                                            return emp?.es_sucursales ? <option value="sucursales">Sucursales</option> : null
                                        })()}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className={estilos.grupoInput}>
                                ...
                                <div className={estilos.selectorModo}>
                                    {(() => {
                                        const modoVisual = getModoVisualFormulario()
                                        if (modoVisual === 'OBRAS') {
                                            return (
                                                <label className={`${estilos.opcionModo} ${estilos.activo}`}>
                                                    <input type="radio" name="system_mode" value="OBRAS" checked readOnly />
                                                    <ion-icon name="construct-outline"></ion-icon>
                                                    <span>OBRAS (Construcción)</span>
                                                </label>
                                            )
                                        }
                                        if (modoVisual === 'SUCURSALES') {
                                            return (
                                                <label className={`${estilos.opcionModo} ${estilos.activo}`}>
                                                    <input type="radio" name="system_mode" value="POS" checked readOnly />
                                                    <ion-icon name="git-compare-outline"></ion-icon>
                                                    <span>SUCURSALES (Transferencias)</span>
                                                </label>
                                            )
                                        }
                                        return (
                            ...
                            <div className={estilos.grupoInput}>
                                <label className={estilos.checkboxLabel}>
                                    <input
                                        type="checkbox"
                                        name="permite_impresion"
                                        checked={formData.permite_impresion}
                                        onChange={(e) => setFormData(prev => ({ ...prev, permite_impresion: e.target.checked }))}
                                    />
                                    <span>Permitir impresión de tickets</span>
                                </label>
                                <p className={estilos.ayudaInput}>Habilita el botón de impresión en el módulo de ventas</p>
                            </div>
