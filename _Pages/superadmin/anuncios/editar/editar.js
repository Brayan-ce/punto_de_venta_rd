...
import { actualizarAnuncio, obtenerAnuncioPorId, obtenerEmpresasYUsuarios } from "../servidor"
...
export default function EditarAnuncio() {
    ...
    useEffect(() => {
        const cargar = async () => {
            const [resA, resEU] = await Promise.all([
                obtenerAnuncioPorId(id),
                obtenerEmpresasYUsuarios()
            ])
            if (resEU.success) { setEmpresas(resEU.empresas); setUsuarios(resEU.usuarios) }
            if (resA.success) {
                const a = resA.anuncio
                const targets = resA.targets || []
                setForm({
                    title: a.title || "",
                    message: a.message || "",
                    type: a.type || "info",
                ...
                if (targets.length > 0 && targets[0].empresa_id) {
                    setEmpresaFiltro(targets[0].empresa_id)
                }
            } else {
                router.push("/superadmin/anuncios")
            }
            setCargando(false)
        }
        if (id) cargar()
    ...
    const seleccionarEmpresa = (empId) => {
        setEmpresaFiltro(empId)
        setForm(f => ({ ...f, targets: [] }))
    }

    const toggleUsuario = (uid) => {
        setForm(f => {
            const existe = f.targets.some(t => t.user_id === uid)
            if (existe) return { ...f, targets: f.targets.filter(t => t.user_id !== uid) }
            const usuario = usuarios.find(u => u.id === uid)
            return { ...f, targets: [...f.targets, { user_id: uid, empresa_id: usuario?.empresa_id }] }
        })
    }
    ...
    const guardar = async (e) => {
        ...
        try {
            ...
            const res = await actualizarAnuncio(id, payload)
        ...
        return (
            <div className={`${estilos.pagina} ${estilos[tema]}`}>
                <div className={estilos.contenedor}>
                    <div className={estilos.cargandoPagina}>
                ...
                <div className={estilos.header}>
                    <div className={estilos.headerFila}>
                        <div className={estilos.headerTexto}>
                            ...
                            <div>
                                <h1 className={estilos.titulo}>Editar anuncio</h1>
                                <p className={estilos.subtitulo}>Modifica el mensaje y su configuracion</p>
                            </div>
                        </div>
                            ...
                            <div className={`${estilos.card} ${estilos[tema]}`}>
                                ...
                                <div className={estilos.cardContenido}>
                                    ...
                                    <label className={estilos.checkLabel}>
                                        ...
                                        <input type="checkbox" checked={form.recurrence === 'monthly'} onChange={(e) => setForm(f => ({ ...f, recurrence: e.target.checked ? 'monthly' : '', day_of_month: e.target.checked ? '1' : '' }))} className={estilos.checkHidden} />
