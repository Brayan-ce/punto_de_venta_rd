...
import { obtenerTodosModulos, obtenerModulosEmpresa, toggleModuloEmpresa, obtenerPermisoImpresionEmpresa, togglePermisoImpresionEmpresa } from './servidor'
...
export default function ModulosEmpresa({ empresaId, nombreEmpresa }) {
    ...
    const [permisoImpresion, setPermisoImpresion] = useState(true)
    ...
    const cargarModulos = async () => {
        setCargando(true)
        try {
            const [todos, empresa, impresion] = await Promise.all([
                obtenerTodosModulos(),
                obtenerModulosEmpresa(empresaId),
                obtenerPermisoImpresionEmpresa(empresaId)
            ])
            ...
            if (impresion.success) {
                setPermisoImpresion(impresion.permite_impresion)
            }
        } catch (error) {
    ...
    const manejarToggleImpresion = async (habilitado) => {
        setProcesandoImpresion(true)
        setMensaje(null)

        try {
            const resultado = await togglePermisoImpresionEmpresa(empresaId, habilitado)

            if (resultado.success) {
                setPermisoImpresion(habilitado)
                setMensaje({ tipo: 'exito', texto: resultado.mensaje })
            } else {
                setMensaje({ tipo: 'error', texto: resultado.mensaje })
            }
        } catch (error) {
            console.error('Error al toggle impresión:', error)
            setMensaje({ tipo: 'error', texto: 'Error al actualizar permiso de impresión' })
        } finally {
    ...
    const categoriasOrdenadas = ['core', 'pos', 'finanzas', 'credito', 'financiamiento', 'constructora', 'sucursales', 'catalogo']
        ...
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            ...
            <div className={estilos.modulos}>
                {categoriasOrdenadas.map(categoria => {
                    ...
                    const posModulo = modulosCategoria.find(m => m.codigo === 'pos')
                    const posHabilitado = posModulo ? obtenerEstadoModulo(posModulo.id) : false
                    ...
                    return (
                        <div key={categoria} className={estilos.categoria}>
                            ...
                            <div className={estilos.listaModulos}>
                                ...
                                {categoria === 'pos' && posHabilitado && (
                                    <div className={`${estilos.configItem} ${permisoImpresion ? estilos.configActivo : ''}`}>
                                        <div className={estilos.configInfo}>
                                            <div className={estilos.configHeader}>
                                                <ion-icon name="print-outline"></ion-icon>
                                                <span className={estilos.configLabel}>Permitir impresión de tickets</span>
                                            </div>
                                            <p className={estilos.configDesc}>
                                                Habilita el botón de impresión y firma e-NCF para todos los usuarios de esta empresa
                                            </p>
                                        </div>
                                        <div className={estilos.moduloAcciones}>
                                            <label className={estilos.switch}>
                                                <input
                                                    type="checkbox"
                                                    checked={permisoImpresion}
                                                    disabled={procesandoImpresion}
                                                    onChange={(e) => manejarToggleImpresion(e.target.checked)}
                                                />
                                                <span className={estilos.slider}></span>
                                            </label>
                                        </div>
