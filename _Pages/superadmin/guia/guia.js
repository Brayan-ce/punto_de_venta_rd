...
const TIPOS = [
    ...
    { value: 'archivo', label: 'Archivo (Descarga)', icon: 'download-outline' },
]
...
const TIPO_ICONS = {
    ...
    archivo: 'download-outline',
}

const FORM_VACIO = { titulo: '', descripcion: '', tipo: 'video', contenido: '', nombre_archivo: '', orden: 0, activo: true }
...
export default function GuiaPage() {
    ...
    const abrirEditar = (guia) => {
        ...
        setForm({
            ...
            nombre_archivo: guia.nombre_archivo || '',
            orden: guia.orden,
    ...
    const handleArchivoFileChange = (e) => {
        const file = e.target.files?.[0]
        if (!file) return
        if (file.size > 200 * 1024 * 1024) {
            mostrarMensaje('El archivo no puede superar los 200 MB', 'error')
            return
        }
        setArchivoFile(file)
        setForm(f => ({ ...f, contenido: '', nombre_archivo: file.name }))
    }
    ...
    // Sube cualquier archivo (documento, apk, zip, etc.) para descarga directa
    const subirArchivoDirecto = (file) => {
        return new Promise((resolve, reject) => {
            const fd = new FormData()
            fd.append('file', file)
            ...
            xhr.open('POST', '/api/upload-guia-archivo')
            xhr.send(fd)
        })
    }
    ...
    const handleGuardar = async () => {
        ...
        if (form.tipo === 'archivo' && archivoFile) {
            setSubiendoArchivo(true)
            setProgresoArchivo(0)
            try {
                contenidoFinal = await subirArchivoDirecto(archivoFile)
            } catch (err) {
                setGuardando(false)
                setSubiendoArchivo(false)
                setProgresoArchivo(0)
                return mostrarMensaje(err.message || 'Error al subir el archivo', 'error')
            }
            setSubiendoArchivo(false)
        }

        if (form.tipo === 'archivo' && !contenidoFinal) {
            setGuardando(false)
            return mostrarMensaje('Debes subir un archivo para descargar', 'error')
        }
    ...
    const renderContenido = (guia) => {
        ...
        if (guia.tipo === 'archivo') {
            return (
                <a href={guia.contenido} download={guia.nombre_archivo || guia.titulo} className={estilos.pdfLink}>
                    <ion-icon name="download-outline"></ion-icon>
                    <span>Descargar {guia.nombre_archivo || guia.titulo}</span>
                </a>
            )
        }
    }
    ...
    const labelContenido = () => {
        switch (form.tipo) {
            ...
            case 'archivo': return 'Archivo para Descargar'
                ...
                <div className={estilos.overlay} onClick={cerrarForm}>
                    <div className={`${estilos.modal} ${estilos[tema]}`} onClick={e => e.stopPropagation()}>
                            ...
                            <div className={estilos.formFila}>
                                <div className={estilos.formGrupo}>
                                    ...
                                    <select
                                        ...
                                        value={form.tipo}
                                        onChange={e => {
                                            setForm(f => ({ ...f, tipo: e.target.value, contenido: '', nombre_archivo: '' }))
                                            setVideoFile(null)
                                            setArchivoFile(null)
                                        }}
                                    >
                                    ...
                                    <div className={estilos.uploadArea}>
                                        ...
                                        {archivoFile ? (
                                            <div className={estilos.videoSeleccionado}>
                                                ...
                                                <div className={estilos.videoInfo}>
                                                    <span className={estilos.videoNombre}>{archivoFile.name}</span>
                                                    <span className={estilos.videoTamano}>
                                                        {(archivoFile.size / (1024 * 1024)).toFixed(1)} MB
                                                    </span>
                                                </div>
                                                <button
                                                    type="button"
                                                    className={estilos.btnQuitarVideo}
                                                    onClick={() => { setArchivoFile(null); setForm(f => ({ ...f, contenido: '', nombre_archivo: '' })) }}
                                                >
                                                    <ion-icon name="close-circle-outline"></ion-icon>
                                                </button>
                                            </div>
                                            ...
                                            <div className={estilos.videoExistente}>
                                                <ion-icon name="checkmark-circle-outline"></ion-icon>
                                                <span>Archivo actual: <strong>{form.nombre_archivo || form.contenido.split('/').pop()}</strong></span>
                                                <button
                                                    type="button"
                                                    className={estilos.btnCambiarVideo}
                                        ...
                                        {subiendoArchivo && (
                                            <div className={estilos.progressBar}>
                                                <div className={estilos.progressFill} style={{ width: `${progresoArchivo}%` }} />
                        ...
                        <div className={estilos.modalFooter}>
                            ...
                            <button className={estilos.btnPrimario} onClick={handleGuardar} disabled={guardando || subiendoVideo || subiendoArchivo}>
                                {subiendoVideo ? `Subiendo... ${progresoSubida}%` : subiendoArchivo ? `Subiendo... ${progresoArchivo}%` : guardando ? 'Guardando...' : editando ? 'Actualizar' : 'Crear Guía'}
                            </button>
                        </div>
                    </div>
                </div>
