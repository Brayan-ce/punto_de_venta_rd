...
export async function obtenerGuias() {
    ...
    try {
        ...
        const [rows] = await connection.execute(
            `SELECT id, titulo, descripcion, tipo, contenido, nombre_archivo, orden, activo, created_at, updated_at
             FROM guia_contenido
...
export async function crearGuia(datos) {
    ...
    try {
        ...
        const { titulo, descripcion, tipo, contenido, nombre_archivo, orden, activo } = datos
        ...
        await conPermisoEscrituraOffline(async () => {
            ...
            ;[result] = await connection.execute(
                `INSERT INTO guia_contenido (titulo, descripcion, tipo, contenido, nombre_archivo, orden, activo)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [titulo.trim(), descripcion?.trim() || null, tipo, contenido?.trim() || null, nombre_archivo?.trim() || null, nuevoOrden, activo ? 1 : 0]
            )
            connection.release()
            connection = null
        })
    ...
    try {
        ...
        const [result] = await connection.execute(
            `UPDATE guia_contenido
             SET titulo = ?, descripcion = ?, tipo = ?, contenido = ?, nombre_archivo = ?, orden = ?, activo = ?
             WHERE id = ?`,
            [titulo.trim(), descripcion?.trim() || null, tipo, contenido?.trim() || null, nombre_archivo?.trim() || null, orden ?? 0, activo ? 1 : 0, id]
        )
        connection.release()
