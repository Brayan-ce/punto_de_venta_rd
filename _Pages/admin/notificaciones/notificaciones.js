...
export default function Notificaciones() {
    ...
    const [data, setData] = useState({ cuotasProximas: [], cuotasVencidas: [], alertas: [], productosPorVencer: [], productosVencidos: [], stats: {} })
    ...
    const TABS = [
        ...
        { key: 'productosPorVencer', dataKey: 'productosPorVencer', label: tr('Productos por vencer', 'Products expiring soon'), icon: 'cube-outline', count: data.stats.productosPorVencer },
        { key: 'productosVencidos',  dataKey: 'productosVencidos',  label: tr('Productos vencidos', 'Expired products'),        icon: 'close-circle-outline', count: data.stats.productosVencidos },
    ]
            ...
            <div className={estilos.header}>
                <div className={estilos.headerInfo}>
                    ...
                    <div>
                        ...
                        <p className={estilos.subtitulo}>{tr('Cuotas, productos por vencer y alertas de financiamiento', 'Installments, expiring products and financing alerts')}</p>
                    </div>
                </div>
            ...
            {lista.length === 0 ? (
                ...
                <div className={estilos.lista}>
                    {lista.map(item => {
                        ...
                        const esProducto = tab === 'productosPorVencer' || tab === 'productosVencidos'
                        const dias = diasPara(item.fecha_vencimiento)
                        const monto = (esAlerta || esProducto) ? 0 : (parseFloat(item.monto) + parseFloat(item.mora || 0))

                        if (esProducto) {
                            const esVencido = tab === 'productosVencidos'
                            return (
                                <div key={item.id} className={`${estilos.item} ${esVencido ? estilos.itemVencida : ''}`}>
                                    <div className={`${estilos.avatar} ${esVencido ? estilos.avatarVencida : ''}`}>
                                        <ion-icon name="cube-outline"></ion-icon>
                                    </div>
                                    <div className={estilos.itemInfo}>
                                        <span className={estilos.itemNombre}>{item.nombre}</span>
                                        <span className={estilos.itemMeta}>
                                            {[item.sku, item.lote ? `${tr('Lote', 'Batch')} ${item.lote}` : null, item.ubicacion_bodega].filter(Boolean).join(' · ')}
                                        </span>
                                    ...
                                    <div className={estilos.itemDerecha}>
                                        <div className={estilos.itemAcciones}>
                                            <Link href={`/admin/productos/ver/${item.id}`} className={estilos.btnAccion} title={tr('Ver producto', 'View product')}>
