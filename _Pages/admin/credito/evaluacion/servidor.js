...
export async function evaluarOtorgamientoCredito(clienteId, montoSolicitado = 0) {
    ...
    try {
        ...
        const [empresaRows] = await connection.execute(
            `SELECT moneda, locale FROM empresas WHERE id = ? AND activo = TRUE`,
            [empresaId]
        )
        const empresaConfig = empresaRows[0] || { moneda: 'DOP', locale: 'es-DO' }
        ...
        // Regla 4: Monto solicitado excede saldo disponible
            ...
            aprobado = false
            motivo = `Monto solicitado (${formatearMoneda(montoSolicitado, empresaConfig.moneda, empresaConfig.locale)}) excede saldo disponible (${formatearMoneda(credito.saldo_disponible, empresaConfig.moneda, empresaConfig.locale)})`
        }

        // Regla 5: Deudas vencidas
        else if (deudas.total_vencidas > 0) {
            aprobado = false
            motivo = `Cliente tiene ${deudas.total_vencidas} factura(s) vencida(s) - ${formatearMoneda(deudas.monto_vencido, empresaConfig.moneda, empresaConfig.locale)}`
        }
