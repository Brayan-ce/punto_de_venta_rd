...
export default function InventarioAdmin() {
    ...
    const calcularNivelStock = (producto) => {
        const max = parseFloat(producto.stock_maximo) || 0
        const actual = parseFloat(producto.stock) || 0
        if (max <= 0) return 0
        return Math.max(2, Math.min(100, Math.round((actual / max) * 100)))
    }
                            ...
                            <div className={estilos.tabla}>
                                ...
                                <div className={estilos.tablaBody}>
                                    {productosPagina.map((producto, index) => (
                                        <div
                                            key={producto.id}
                                            className={`${estilos.fila} ${estilos[tema]}`}
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                <div className={estilos.stockBarTrack}>
                                                    <div
                                                        className={`${estilos.stockBarFill} ${producto.stock === 0 ? estilos.agotado : producto.stock <= producto.stock_minimo ? estilos.bajo : estilos.normal}`}
                                                        style={{ width: `${calcularNivelStock(producto)}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                            <div>
                            ...
                            <div className={estilos.listaMovimientos}>
                                {movimientosPagina.map((movimiento, index) => {
                                    ...
                                    return (
                                        <div
                                            key={movimiento.id}
                                            className={`${estilos.movimientoItem} ${estilos[tema]} ${estilos['borde_' + tipoInfo.color]}`}
                                            style={{ animationDelay: `${Math.min(index, 12) * 35}ms` }}
                                        >
