...
export default function PerfilAdmin() {
    ...
    const cargarPerfil = async () => {
        ...
        try {
            ...
            if (resultado.success) {
                ...
                alert(resultado.mensaje || tr('Error al cargar perfil', 'Error loading profile'))
            }
        } catch (error) {
    ...
    const validarFormulario = () => {
        ...
        if (!datosPerfil.nombre.trim()) {
            erroresTemp.nombre = tr('El nombre es requerido', 'Name is required')
        }

        if (datosPerfil.password && datosPerfil.password.length < 6) {
            erroresTemp.password = tr('La contrasena debe tener al menos 6 caracteres', 'Password must be at least 6 characters')
        }
    ...
    const manejarCambioAvatar = (e) => {
        ...
        if (archivo) {
            if (archivo.size > 5 * 1024 * 1024) {
                alert(tr('El archivo es muy grande. Maximo 5MB', 'File is too large. Maximum 5MB'))
                return
            }
            if (!archivo.type.startsWith('image/')) {
                alert(tr('Solo se permiten imagenes', 'Only images are allowed'))
                return
            }
    ...
    const aplicarRecorte = async () => {
        try {
            ...
            alert(tr('Error al recortar la imagen', 'Error cropping image'))
        }
    }
        ...
        try {
            ...
            if (archivoAvatar) {
                ...
                if (resultadoImagen.success) {
                    ...
                    alert(tr('Error al subir el avatar', 'Error uploading avatar'))
                    setProcesando(false)
                    return
                }
            }
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            <div className={estilos.header}>
                <div>
                    <h1 className={estilos.titulo}>{tr('Mi Perfil', 'My Profile')}</h1>
                    <p className={estilos.subtitulo}>{tr('Actualiza tu informacion personal', 'Update your personal information')}</p>
                </div>
            </div>
            ...
            <div className={`${estilos.seccion} ${estilos[tema]}`}>
                <div className={estilos.seccionHeader}>
                    ...
                    <div>
                        <h2 className={estilos.seccionTitulo}>{tr('Informacion Personal', 'Personal Information')}</h2>
                        <p className={estilos.seccionSubtitulo}>{tr('Gestiona tus datos de usuario', 'Manage your user data')}</p>
                    </div>
                </div>

                <form onSubmit={manejarSubmit} className={estilos.formulario}>
                    <div className={estilos.grupoImagen}>
                        <label className={estilos.labelImagen}>{tr('Foto de Perfil', 'Profile Picture')}</label>
                    ...
                    <div className={estilos.grupoInput}>
                        ...
                        <input
                            ...
                            placeholder={tr('Ej: Juan Perez', 'E.g.: John Doe')}
                            disabled={procesando}
                        />
                        {errores.nombre && (
                    ...
                    <div className={estilos.filaForm}>
                        <div className={estilos.grupoInput}>
                            ...
                            <input
                                ...
                                disabled
                            />
                            <span className={estilos.textoAyuda}>{tr('No se puede modificar', 'Cannot be modified')}</span>
                        </div>
                    ...
                    <div className={estilos.grupoInput}>
                        ...
                        <div className={estilos.passwordWrapper}>
                            <input
                                ...
                                placeholder={tr('Minimo 6 caracteres', 'Minimum 6 characters')}
                                disabled={procesando}
                            />
                            <button
                                type="button"
                ...
                <div className={estilos.modalRecorte}>
                    <div className={`${estilos.modalRecorteContenido} ${estilos[tema]}`}>
                        <div className={estilos.modalRecorteHeader}>
                            <h3>{tr('Recortar Foto de Perfil', 'Crop Profile Picture')}</h3>
