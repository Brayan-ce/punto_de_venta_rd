    ...
    try {
        ...
        if (ids.length > 0) {
            ...
            const [cuotas] = await connection.execute(
                `SELECT fc.*, COALESCE(SUM(fpc.monto), 0) AS monto_pagado
                 FROM fin_cuotas fc
                 LEFT JOIN fin_pago_cuotas fpc ON fpc.cuota_id = fc.id
                 INNER JOIN (
                     SELECT fc2.contrato_id, MIN(fc2.numero) AS min_numero
                     FROM fin_cuotas fc2
                     WHERE fc2.contrato_id IN (${ph})
                       AND (
                           fc2.estado IN ('pendiente','vencida','parcial')
                           OR fc2.monto > COALESCE((
                               SELECT SUM(fpc2.monto)
                               FROM fin_pago_cuotas fpc2
                               WHERE fpc2.cuota_id = fc2.id
                           ), 0)
                       )
                     GROUP BY fc2.contrato_id
        ...
        for (const cu of proximasCuotas) {
            ...
            cuotaMap[cu.contrato_id] = {
                ...cu,
                estado: cu.estado === 'pagada' && montoRestante > 0 ? 'pendiente' : cu.estado,
                mora: moraActual,
                monto_restante: montoRestante > 0 ? montoRestante : parseFloat(cu.monto)
            }
        }
        ...
        try {
            ...
            const [todasCuotas] = await connection.execute(
                `SELECT fc.*, p.mora_pct, p.dias_gracia
                 ...
                 WHERE fc.contrato_id = ?
                   AND (
                       fc.estado IN ('pendiente','vencida','parcial')
                       OR fc.monto > COALESCE((
                           SELECT SUM(pc.monto)
                           FROM fin_pago_cuotas pc
                           WHERE pc.cuota_id = fc.id
                       ), 0)
                   )
            ...
            if (cuotasAplicadas.length === 0) {
                ...
                connection.release()
                return { success: false, mensaje: 'No hay cuotas con saldo pendiente para aplicar este pago' }
            }
            ...
            const cuotasPagadasIds = cuotasAplicadas.filter(c => c.estado === 'pagada').map(c => c.id)
            if (cuotasPagadasIds.length > 0) {
                const marcadores = cuotasPagadasIds.map(() => '?').join(', ')
                await connection.execute(
                    `UPDATE fin_alertas SET estado = 'resuelta' WHERE cuota_id IN (${marcadores}) AND estado = 'activa'`,
                    cuotasPagadasIds
                )
            }
            ...
            if (estadoContrato === 'pagado') {
                await connection.execute(
                    `UPDATE fin_alertas SET estado = 'resuelta' WHERE contrato_id = ? AND estado = 'activa'`,
                    [cuota.contrato_id]
                )
            }
