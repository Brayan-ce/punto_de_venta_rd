/**
 * Servicio de Dominio: Planes de Financiamiento
 * 
 ...
 * 
 * REFACTORIZADO: Ahora separa planes comerciales (cash/diferido) de planes financieros (crédito)
 */
...
export class PlanService {
   ...
   * 
   * @param {Object} datosPlazo - { plazo_meses, tipo_pago_inicial, pago_inicial_valor, cuota_mensual, recargo_tipo?, recargo_valor?, precio_producto? }
  ...
  static calcularPlazoComercial(datosPlazo, politica = null) {
    ...
    // Si no hay política ni datos, usar valores por defecto según el plazo
    let recargoTipo = politica?.recargo_tipo || datosPlazo.recargo_tipo
    let recargoValor = politica?.recargo_valor !== null && politica?.recargo_valor !== undefined
      ? politica.recargo_valor
      : (datosPlazo.recargo_valor !== null && datosPlazo.recargo_valor !== undefined
          ? datosPlazo.recargo_valor
          : null)
    
    // Si no hay recargo definido, usar valores por defecto según el plazo
    if (!recargoTipo || recargoValor === null || recargoValor === undefined || recargoValor <= 0) {
      const plazoMeses = datosPlazo.plazo_meses
      if (plazoMeses <= 2) {
        recargoTipo = 'FIJO'
        recargoValor = 800
    ...
    if (!recargoTipo || !recargoValor || recargoValor <= 0) {
      recargoTipo = 'FIJO'
      recargoValor = 1000
    }
  ...
  static validarDatosPlazo(plazo) {
    ...
    if (tipoPlan === 'COMERCIAL') {
      ...
      // Validar recargo solo si está presente (si no está, se asignará desde la política al guardar)
      if (plazo.recargo_tipo && (plazo.recargo_valor === null || plazo.recargo_valor === undefined || plazo.recargo_valor <= 0)) {
        errores.push('Si se especifica tipo de recargo, debe proporcionarse un valor mayor a 0')
      }
      // Si no hay recargo, no es error (se asignará desde política al guardar en servidor)
    } else {
