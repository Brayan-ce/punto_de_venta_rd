    ...
    try {
        ...
        const [[stats]] = await connection.query(`
            SELECT
                ...
                COALESCE(SUM(CASE WHEN cc.activo = TRUE THEN cc.limite_credito END), 0) AS creditoOtorgado,
                COALESCE(SUM(CASE WHEN cc.activo = TRUE THEN cc.saldo_disponible END), 0) AS creditoDisponible,
                COUNT(CASE WHEN COALESCE(cc.estado_credito, 'normal') = 'normal' THEN 1 END) AS clientesNormales,
                ...
                COUNT(CASE WHEN COALESCE(cc.clasificacion, 'A') = 'A' THEN 1 END)      AS clasificacionA,
            ...
            FROM clientes c
            LEFT JOIN credito_clientes cc
                ON cc.cliente_id = c.id
               AND cc.empresa_id = c.empresa_id
               AND cc.activo = TRUE
            WHERE c.empresa_id = ? AND c.estado IN ('activo', 'inactivo')
        `, [empresaId])
