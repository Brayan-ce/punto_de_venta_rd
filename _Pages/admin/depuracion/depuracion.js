    ...
    return (
        <div>
            ...
            {cargando ? (
                ...
                <div className={estilos.clientesGrid}>
                    {clientes.map(cliente => (
                        <article key={cliente.id} className={`${estilos.clienteCard} ${estilos[tema]}`}>
                            <div className={estilos.clienteCardTop}>
                                <div className={estilos.clienteAvatarGrande}>
                                    {cliente.fotoUrl
                            ...
                            <p className={estilos.clienteDocumento}>{cliente.numeroDocumento || tr('Sin documento', 'No ID')}</p>
                            <p className={estilos.clienteEmpresa}>{cliente.telefono || tr('Sin teléfono registrado', 'No phone registered')}</p>
                            <div className={estilos.clienteScore}>
                                <span className={estilos.estrellas}>★★★★★</span>
                                <strong>{cliente.scoreCrediticio}</strong>
                                <span>{tr('calificación', 'score')}</span>
                            </div>
                            <div className={estilos.clienteCardMeta}>
                                <span className={`${estilos.badgeEstado} ${estilos[cliente.estadoCredito]}`}>
                                    {cliente.estadoCredito === 'normal' ? tr('normal', 'normal') : cliente.estadoCredito === 'atrasado' ? tr('atrasado', 'overdue') : cliente.estadoCredito}
                                </span>
                                <span>{cliente.cuotasVencidas || 0} {tr('vencidas', 'overdue')}</span>
                            </div>
                            <button className={estilos.btnVerCliente} onClick={() => router.push(`/admin/depuracion/ver?id=${cliente.id}`)}>
                                <ion-icon name="eye-outline"></ion-icon>
                                {tr('Ver cliente', 'View customer')}
                            </button>
                        </article>
                    ))}
                    {clientes.length === 0 && (
