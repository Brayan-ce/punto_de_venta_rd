...
import {reproducirTono, desbloquearAudio} from '@/lib/sonidos/tonos'
import {
    NAVIGATION_CATALOG,
    obtenerItemsTop,
...
export default function HeaderAdmin() {
    ...
    const prevTotalNotifRef = useRef(null)
    ...
    useEffect(() => {
        try {
            const guardadas = JSON.parse(localStorage.getItem('notificacionesDescartadas') || '[]')
            setNotificacionesDescartadas(new Set(Array.isArray(guardadas) ? guardadas : []))
        } catch {
            setNotificacionesDescartadas(new Set())
        }
    }, [])
    ...
    const labelMapping = {
      ...
      'Inventario': 'header.inventario',
            ...
            'Libro de Banco': 'header.libroBanco',
            'Activos Fijos': 'header.activosFijos',
    ...
    const esSoloDelivery = datosUsuario?.tipo === 'delivery'
    ...
    const obtenerDashboardPrincipal = useMemo(() => {
        if (esSoloDelivery) return '/admin/delivery'
    ...
    }, [pathname, tieneModuloHeader, userSystemMode, esSoloFinanciamiento, esSoloSucursales, esSoloDelivery])
    ...
    const navegacionPrincipal = useMemo(() => {
        if (esSoloDelivery) return [{ href: '/admin/delivery', icon: 'navigate-outline', label: 'Delivery' }]
        if (esSoloSucursales) {
        ...
        return items
    }, [tieneModuloHeader, systemMode, esSoloFinanciamiento, esSoloSucursales, esSoloDelivery, pathname])
    ...
    const categoriasNavegacion = useMemo(() => {
        if (esSoloDelivery) {
            return [{
                ...
                modulo: 'delivery',
                systemMode: 'POS',
                uniqueKey: 'delivery-only',
                items: [{ href: '/admin/delivery', icon: 'navigate-outline', label: 'Delivery', modulo: 'delivery', top: true }]
            }]
        }
        if (esSoloSucursales) {
    ...
    const accionesDiarias = useMemo(() => {
        ...
        if (esSoloFinanciamiento || esSoloSucursales || esSoloDelivery) return []
        return obtenerAccionesDiarias(tieneModuloHeader, systemMode)
    }, [tieneModuloHeader, systemMode, esSoloFinanciamiento, esSoloSucursales, esSoloDelivery])

    const basePathApp = esSoloFinanciamiento ? '/financiamiento' : esSoloSucursales ? '/sucursales' : '/admin'
    const mostrarNotificaciones = !esSoloSucursales && !esSoloDelivery && systemMode !== 'OBRAS'
    ...
    const cargarNotificaciones = useCallback(async () => {
        ...
        try {
            ...
            if (res.success) {
                const filtrar = (items, tipo) => (items || []).filter((item) => {
                    const clave = `${tipo}:${item.id}`
                    const claveAnteriorAlerta = tipo === 'alerta' ? `vencida:${item.id}` : null
                    return !notificacionesDescartadas.has(clave) && (!claveAnteriorAlerta || !notificacionesDescartadas.has(claveAnteriorAlerta))
                })
                const cuotasProximas = filtrar(res.cuotasProximas, 'proxima')
                const cuotasVencidas = filtrar(res.cuotasVencidas, 'vencida')
                const alertas = filtrar(res.alertas, 'alerta')
                const productosPorVencer = filtrar(res.productosPorVencer, 'producto-proximo')
                const productosVencidos = filtrar(res.productosVencidos, 'producto-vencido')
                setNotifData({
                    cuotasProximas,
                    cuotasVencidas,
                    alertas,
                    ...
                    productosVencidos,
                    stats: {
                        proximas: cuotasProximas.length,
                ...
                const nuevoTotal = cuotasProximas.length + cuotasVencidas.length + alertas.length + productosPorVencer.length + productosVencidos.length
                setTotalNotif(nuevoTotal)

                const totalAnterior = prevTotalNotifRef.current
                if (res.config?.sonidoActivo && totalAnterior !== null && nuevoTotal > totalAnterior) {
                    reproducirTono(res.config.sonidoTono)
                }
                prevTotalNotifRef.current = nuevoTotal
            }
        } catch (e) {
    ...
    }, [mostrarNotificaciones, notificacionesDescartadas])
    ...
    // Los navegadores bloquean el audio hasta que el usuario interactua con la
    // pagina; esta escucha "desbloquea" el sonido de notificaciones en cuanto ocurre.
    useEffect(() => {
        const manejarPrimeraInteraccion = () => desbloquearAudio()
        window.addEventListener('click', manejarPrimeraInteraccion, { once: true })
        window.addEventListener('keydown', manejarPrimeraInteraccion, { once: true })
        window.addEventListener('touchstart', manejarPrimeraInteraccion, { once: true })
        return () => {
            window.removeEventListener('click', manejarPrimeraInteraccion)
            window.removeEventListener('keydown', manejarPrimeraInteraccion)
            window.removeEventListener('touchstart', manejarPrimeraInteraccion)
        }
    }, [])
    ...
    const irNotificacion = (notif, tipo = 'vencida') => {
        ...
        setNotificacionesDescartadas((anteriores) => {
            ...
            siguiente.add(clave)
            localStorage.setItem('notificacionesDescartadas', JSON.stringify([...siguiente]))
            return siguiente
        })
        setTotalNotif((total) => Math.max(0, total - 1))
        setMenuNotifAbierto(false)
        router.push(`${basePathApp}/clientes/ver/${notif.cliente_id}?tab=pagos`)
    }

    const descartarNotificacion = (tipo, item) => {
        const clave = `${tipo}:${item.id}`
        setNotificacionesDescartadas((anteriores) => {
            const siguiente = new Set(anteriores)
            siguiente.add(clave)
                                    ...
                                    <>
                                        ...
                                        <div className={`${estilos.notifPanel} ${estilos[tema]}`} role="dialog" aria-label={t('header.notifTitulo')}>
                                            ...
                                            <div className={estilos.notifLista}>
                                                {cargandoNotif && !notifData.cuotasProximas.length && !notifData.cuotasVencidas.length && !notifData.alertas.length && !notifData.productosPorVencer.length && !notifData.productosVencidos.length ? (
                                                    ...
                                                    (() => {
                                                        ...
                                                        const tabActiva = (notifTab === 'proximas' && notifData.config?.mostrarProximas === false)
                                                            ...
                                                            ? tabValida
                                                            : notifTab
                                                        const esProducto = tabActiva === 'productosPorVencer' || tabActiva === 'productosVencidos'
                                                        const items = tabActiva === 'proximas'
                                                            ? notifData.cuotasProximas
                                                            : tabActiva === 'vencidas'
                                                                ? notifData.cuotasVencidas
                                                                : tabActiva === 'productosPorVencer'
                                                                    ? notifData.productosPorVencer
                                                                    : tabActiva === 'productosVencidos'
                                                                        ? notifData.productosVencidos
                                                                        : notifData.alertas
                                                        ...
                                                        if (esProducto) {
                                                            return items.map((prod) => (
                                                                <button
                                                                    key={`prod-${prod.id}`}
                                                                    type="button"
                                                                    className={`${estilos.notifItem} ${tabActiva === 'productosVencidos' ? estilos.notifItemVencida : ''}`}
                                                                    onClick={() => { descartarNotificacion(tabActiva === 'productosVencidos' ? 'producto-vencido' : 'producto-proximo', prod); setMenuNotifAbierto(false); router.push(`${basePathApp}/productos/ver/${prod.id}`) }}
                                                                >
                                                                    <div className={estilos.notifAvatar}>
                                                                        <ion-icon name="cube-outline"></ion-icon>
                                                                    </div>
                                                                    <div className={estilos.notifContenido}>
                                                                        <div className={estilos.notifLineaTop}>
                                                                            <span className={estilos.notifNombre}>{prod.nombre}</span>
                                                                            <span className={estilos.notifTiempo}>{formatearFechaNotif(prod.fecha_vencimiento)}</span>
                                                                        </div>
                                                                        <div className={estilos.notifDetalle}>
                                                                            <span className={`${estilos.notifTag} ${estilos.notifTagFin}`}>
                                                                                {[prod.sku, prod.ubicacion_bodega].filter(Boolean).join(' · ')}
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                        ...
                                                        return items.map((notif) => {
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    onClick={() => notif.cliente_id
                                                                        ? irNotificacion(notif, esAlerta ? 'alerta' : tabActiva === 'proximas' ? 'proxima' : 'vencida')
                                                                        : (descartarNotificacion('alerta', notif), setMenuNotifAbierto(false))}
                                                                >
                                ...
                                {sidebarColapsado ? (
                                    <>
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.preventDefault();
                                                const destino = categoria.items[0]?.href || obtenerDashboardPrincipal;
                                                setHoverSubmenu(null);
