...
export function obtenerTextoEstado(estado, language = 'es') {
    const textos = {
        [ESTADOS_CONDUCE.EMITIDO]: { es: 'Emitido', en: 'Issued' },
        [ESTADOS_CONDUCE.ENTREGADO]: { es: 'Entregado', en: 'Delivered' },
        [ESTADOS_CONDUCE.ANULADO]: { es: 'Anulado', en: 'Canceled' }
    }
    return textos[estado]?.[language === 'en' ? 'en' : 'es'] || estado
}
 ...
 * @returns {string} Texto legible
 */
export function obtenerTextoTipoOrigen(tipoOrigen, language = 'es') {
    const textos = {
        [TIPOS_ORIGEN.VENTA]: { es: 'Venta', en: 'Sale' },
        [TIPOS_ORIGEN.COTIZACION]: { es: 'Cotizacion', en: 'Quote' }
    }
    return textos[tipoOrigen]?.[language === 'en' ? 'en' : 'es'] || tipoOrigen
}
