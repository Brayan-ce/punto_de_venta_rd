    ...
    try {
        ...
        const [rows] = await connection.execute(
            `SELECT c.id,
                    ...
                    -- Subconsulta agregada de deudas (solo CxC para POS)
                    ...
                    COALESCE(fin.saldo_fin, 0)        AS deuda_financiamiento,
                    COALESCE(fin.contratos_activos, 0) AS contratos_activos
             ...
             FROM clientes c
                      ...
                      LEFT JOIN (SELECT fc.cliente_id,
                                        SUM(fc.saldo_pendiente)  AS saldo_fin,
                                        COUNT(fc.id)             AS contratos_activos
                                 FROM fin_contratos fc
                                 WHERE fc.estado = 'activo'
                                 GROUP BY fc.cliente_id) fin ON fin.cliente_id = c.id
        ...
        const clientesFormateados = clientesArray.map((c) => {
            ...
            const utilizadoCxC = Number(c.deuda_total) || 0; // solo cuentas por cobrar
            const disponibleCxC = Math.max(0, limite - utilizadoCxC);
            const porcentajeUso = limite > 0 ? Math.min((utilizadoCxC / limite) * 100, 100) : 0;
            ...
            return {
                ...
                credito: {
                    ...
                    limite,
                    utilizado: utilizadoCxC,
                    disponible: disponibleCxC,
                        ...
                        c.estado_credito === "normal"
                            ...
                            : c.estado_credito === "atrasado"
                                ...
                                : c.estado_credito === "bloqueado"
                                    ? "danger"
                                    : "secondary",
                    puedeVender: c.cliente_estado === "activo",
                },
...
export async function validarClienteParaVenta(clienteId, montoVenta) {
    ...
    try {
        ...
        // Validaciones de crédito: solo perfil existente (sin bloquear por límite, deuda o estado)
        if (cliente.estado_credito === 'bloqueado') {
            return {
                success: true,
                valido: false,
                mensaje: "El crédito del cliente está bloqueado (embargado manualmente)",
