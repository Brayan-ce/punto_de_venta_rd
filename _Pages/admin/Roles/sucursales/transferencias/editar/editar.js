...
import { guardarCambios, obtenerOpciones, obtenerProductosOrigen, obtenerTransferencia } from './servidor'
import estilos from './editar.module.css'
import LoadingScreen from '@/_EXTRAS/Componentes/LoadingScreen/LoadingScreen'

export default function EditarTransferenciaSucursal() {
    const { id } = useParams()
    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            const [resOpciones, resTransferencia] = await Promise.all([
                obtenerOpciones(),
                obtenerTransferencia(id)
            ])

            if (resOpciones?.success) {
                setSucursales(resOpciones.sucursales || [])
            }

            if (!resTransferencia?.success) {
                setEstado({ tipo: 'error', texto: resTransferencia?.mensaje || tr('No se pudo cargar', 'Could not load') })
                setCargando(false)
                return
            }

            const t = resTransferencia.transferencia
            setFormulario({
                sucursal_origen_id: String(t.sucursal_origen_id || ''),
                sucursal_destino_id: String(t.sucursal_destino_id || ''),
                prioridad: t.prioridad || 'normal',
                observacion_origen: t.observacion_origen || '',
                estado_actual: t.estado || 'pendiente'
            })

            setDetalle((resTransferencia.detalle || []).map((d) => ({
        ...
        const cargarProductos = async () => {
            ...
            if (res?.success) {
                ...
                setDetalle((prev) => prev.map((d) => {
                    const encontrado = (res.productos || []).find((p) => Number(p.id) === Number(d.producto_id))
                    return {
                        ...d,
                        stock_disponible: Number(encontrado?.stock_disponible || 0)
                    }
                }))
            }
        }

        if (!cargando) cargarProductos()
    }, [formulario.sucursal_origen_id, cargando])
    ...
    const productoSeleccionado = useMemo(() => {
        ...
        return productosOrigen.find((p) => Number(p.id) === idProducto) || null
    ...
    const guardar = async (e) => {
        ...
        const res = await guardarCambios(id, {
            ...formulario,
        ...
        router.push(`/sucursales/transferencias/${id}`)
    }
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
            <div className={estilos.header}>
                <div>
                    <h1 className={estilos.titulo}>{tr('Editar Transferencia', 'Edit Transfer')}</h1>
                    <p className={estilos.subtitulo}>{tr('Modifica origen, destino y productos de la transferencia', 'Edit origin, destination and transfer products')}</p>
                </div>
                <Link href={`/sucursales/transferencias/${id}`} className={estilos.btnSecundario}>{tr('Cancelar', 'Cancel')}</Link>
            </div>

            {formulario.estado_actual !== 'pendiente' && (
                <div className={`${estilos.alerta} ${estilos.error}`}>{tr('Solo puedes editar transferencias en estado pendiente', 'You can only edit pending transfers')}</div>
            )}
            ...
            <form onSubmit={guardar} className={estilos.form}>
                ...
                <div className={estilos.botones}>
                    <button type="submit" className={estilos.btn} disabled={guardando || formulario.estado_actual !== 'pendiente'}>
                        {guardando ? tr('Guardando...', 'Saving...') : tr('Guardar Cambios', 'Save Changes')}
                    </button>
                </div>
            </form>
        </div>
