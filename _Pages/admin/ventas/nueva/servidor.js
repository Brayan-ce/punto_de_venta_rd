...
export async function crearVenta(datosVenta) {
    ...
    try {
        ...
        if (!Array.isArray(datosVenta.productos) || datosVenta.productos.some(producto => {
            const precio = Number(producto.precio_unitario_usado ?? producto.precio_unitario)
            return !Number.isFinite(precio) || precio <= 0
        })) {
            return { success: false, mensaje: 'Cada producto debe tener un precio unitario mayor que cero' }
        }
        const totalVenta = Number(datosVenta.total)
        if (!Number.isFinite(totalVenta) || totalVenta < 0) {
            return { success: false, mensaje: 'El total de la venta no puede ser negativo. Revisa los descuentos aplicados.' }
        }
        ...
        for (const producto of datosVenta.productos) {
            ...
            const precioPorUnidad = Number.isFinite(Number(producto.precio_unitario_usado))
                ? Number(producto.precio_unitario_usado)
                : Number.isFinite(Number(producto.precio_unitario))
                    ? Number(producto.precio_unitario)
                    : parseFloat(productoInfo[0].precio_por_unidad)
