    ...
    const cargarDatosIniciales = async () => {
        try {
            ...
            if (res.success) {
                ...
                setCajaActiva(res.cajaActiva || null)
    ...
    const actualizarPrecio = (productoId, nuevoPrecio) => {
        ...
        setProductosVenta(productosVenta.map(p => {
            ...
            return {
                ...p,
                precio_venta_usado: precio,
                subtotal_calculado: (parseFloat(p.cantidad) || 0) * precio
            }
        }))
    }

    const abrirEditorPrecio = (producto) => {
        setProductoPrecioEditando(producto)
        setPrecioTemporal(String(producto.precio_venta_usado || producto.precio_por_unidad || producto.precio_venta || ''))
    }

    const abrirEditorPrecioSeleccionado = () => {
        const producto = productosVenta[filaSeleccionada]
        if (!producto) {
            alert(tr('Selecciona un producto de la lista para ajustar su precio', 'Select a product from the list to adjust its price'))
            return
        }
        abrirEditorPrecio(producto)
    }

    const guardarPrecioAjustado = () => {
        const precio = parseFloat(precioTemporal)
        if (!Number.isFinite(precio) || precio <= 0) {
            alert(tr('Ingresa un precio mayor que cero', 'Enter a price greater than zero'))
            return
        }
        actualizarPrecio(productoPrecioEditando.id, precio)
        setProductoPrecioEditando(null)
        setPrecioTemporal('')
    }

    const restablecerPrecioCatalogo = () => {
        if (!productoPrecioEditando) return
        const precioCatalogo = parseFloat(productoPrecioEditando.precio_por_unidad || productoPrecioEditando.precio_venta || 0)
        if (precioCatalogo > 0) actualizarPrecio(productoPrecioEditando.id, precioCatalogo)
        setProductoPrecioEditando(null)
        setPrecioTemporal('')
    }
    ...
    return (
        <div className={`${estilos.contenedorOptimizado} ${estilos[tema]}`}>
            <header className={estilos.barraHerramientas}>
                ...
                <div className={estilos.barraMeta}>
                    ...
                    <span className={estilos.metaChip}>
                        ...
                        {tr('Caja', 'Register')} {cajaActiva?.numero_caja || '—'}
                    </span>
                </div>
            ...
            <div className={estilos.cuerpoVenta}>
                ...
                <div className={estilos.bloqueComprobante}>
                    ...
                    <aside className={estilos.panelCobro}>
                        ...
                        <button
                            type="button"
                            className={estilos.btnAjustarPrecioResumen}
                            onClick={abrirEditorPrecioSeleccionado}
                            disabled={productosVenta.length === 0}
                        >
                            <ion-icon name="pricetag-outline" />
                            <span>{filaSeleccionada !== null && productosVenta[filaSeleccionada]
                                ? `${tr('Ajustar precio:', 'Adjust price:')} ${productosVenta[filaSeleccionada].nombre}`
                                : tr('Selecciona un producto para ajustar su precio', 'Select a product to adjust its price')}</span>
                        </button>
            ...
            {productoPrecioEditando && (
                <div className={estilos.modalOverlay} onClick={() => setProductoPrecioEditando(null)}>
                    <div className={`${estilos.modal} ${estilos[tema]} ${estilos.modalPrecio}`} onClick={e => e.stopPropagation()}>
                        <div className={estilos.modalHeader}>
                            <h2>{tr('Ajustar precio', 'Adjust price')}</h2>
                            <button className={estilos.btnCerrarModal} onClick={() => setProductoPrecioEditando(null)} type="button">
                                <ion-icon name="close-outline" />
                            </button>
                        </div>
                        <div className={estilos.modalBody}>
                            <p className={estilos.infoModal}>{productoPrecioEditando.nombre}</p>
                            <div className={estilos.precioCatalogoReferencia}>
                                <span>{tr('Precio de catálogo', 'Catalog price')}</span>
                                <strong>{formatearMonto(productoPrecioEditando.precio_por_unidad || productoPrecioEditando.precio_venta || 0)}</strong>
                            </div>
                            <div className={estilos.grupoInput}>
                                <label>{tr('Nuevo precio unitario', 'New unit price')}</label>
                                <input
                                    type="text"
                                    inputMode="decimal"
                                    value={precioTemporal}
                                    onChange={e => setPrecioTemporal(normalizarEntradaDecimal(e.target.value))}
                                    onKeyDown={e => { if (e.key === 'Enter') guardarPrecioAjustado() }}
                        ...
                        <div className={estilos.modalFooter}>
                            <button type="button" className={estilos.btnCancelarModal} onClick={restablecerPrecioCatalogo}>
                                {tr('Restablecer', 'Reset')}
                            </button>
                            <button type="button" className={estilos.btnGuardarModal} onClick={guardarPrecioAjustado}>
                                {tr('Aplicar precio', 'Apply price')}
                            </button>
                        </div>
                    </div>
                </div>
