    ...
    try {
        ...
        for (const productoAnterior of productosAnteriores) {
            const [stockAntes] = await connection.execute(
                ...
                [productoAnterior.producto_id, empresaId]
            )
            ...
            if (stockAntes.length === 0) {
                ...
                return {
                    success: false,
                    mensaje: 'Producto anterior no encontrado'
                }
            }

            const stockAnterior = Number.parseFloat(stockAntes[0].stock)
            const cantidadDevuelta = Number.parseFloat(productoAnterior.cantidad)
            const stockNuevo = stockAnterior + cantidadDevuelta

            await connection.execute(
                `UPDATE productos 
                SET stock = stock + ? 
                WHERE id = ? AND empresa_id = ?`,
                [cantidadDevuelta, productoAnterior.producto_id, empresaId]
            )
        ...
        for (const producto of datosVenta.productos) {
            ...
            const stockDisponible = Number.parseFloat(stockActual[0].stock)
            const cantidadNueva = Number.parseFloat(producto.cantidad)
        ...
        for (const producto of datosVenta.productos) {
            const cantidadNueva = Number.parseFloat(producto.cantidad)
            const precioUnitario = Number.parseFloat(producto.precio_unitario)
            const subtotalProducto = cantidadNueva * precioUnitario
            ...
            await connection.execute(
                ...
                SET stock = stock - ? 
                WHERE id = ? AND empresa_id = ?`,
                [cantidadNueva, producto.producto_id, empresaId]
            )
            ...
            const stockNuevo = Number.parseFloat(productoActualizado[0].stock)
            const stockAnterior = stockNuevo + cantidadNueva
