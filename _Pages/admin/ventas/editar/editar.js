...
export default function EditarVentaAdmin({ returnPath = '/admin/ventas' }) {
    ...
    const actualizarPrecio = (productoId, nuevoPrecio) => {
        setProductosVenta(productosVenta.map(p =>
            p.id === productoId
                ? { ...p, precio_venta_usado: nuevoPrecio === '' ? '' : parseFloat(nuevoPrecio) }
                : p
        ))
    }
    ...
    const validarVenta = () => {
        ...
        if (productosVenta.some(p => !Number.isFinite(Number.parseFloat(p.precio_venta_usado)) || Number.parseFloat(p.precio_venta_usado) <= 0)) {
            alert('Cada producto debe tener un precio mayor que cero')
            return false
        }
