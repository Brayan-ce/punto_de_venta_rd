...
export default function VentaRapida({ returnPath = '/admin/ventas' }) {
    ...
    const efectivoIngresado = metodoPago !== 'efectivo' || efectivoRecibido !== ''
    ...
    function agregarAlCarrito(producto) {
        ...
        setCarrito(prev => {
            ...
            return [...prev, { ...producto, precio_venta_original: producto.precio_venta, cantidad: 1 }]
        })
    }
    ...
    function abrirAjustePrecio(item) {
        const producto = item || carrito[0]
        if (!producto) {
            notificar('warning', 'Agrega un producto antes de ajustar el precio')
            return
        }
        setProductoPrecioEditando(producto)
        setPrecioTemporal(String(producto.precio_venta || ''))
    }

    function aplicarPrecioAjustado() {
        const precio = parseFloat(precioTemporal)
        if (!Number.isFinite(precio) || precio <= 0) {
            notificar('warning', 'Ingresa un precio mayor que cero')
            return
        }
        setCarrito(prev => prev.map(item => item.id === productoPrecioEditando.id
            ? { ...item, precio_venta: precio }
            : item
        ))
        setProductoPrecioEditando(null)
        setPrecioTemporal('')
        notificar('success', 'Precio ajustado para esta venta')
    }

    function restablecerPrecioCatalogo() {
        if (!productoPrecioEditando) return
        const precioCatalogo = parseFloat(productoPrecioEditando.precio_venta_original || productoPrecioEditando.precio_venta || 0)
        if (precioCatalogo > 0) {
            setCarrito(prev => prev.map(item => item.id === productoPrecioEditando.id
                ? { ...item, precio_venta: precioCatalogo }
                : item
            ))
        }
        setProductoPrecioEditando(null)
    ...
    return (
        <div className={`${styles.container} ${styles[tema]}`}>
                ...
                <div className={styles.summarySection}>
                    <div className={styles.camposVenta}>
                        <div className={styles.campoCompacto}>
                            <label htmlFor="metodo-pago-rapida">Método de pago</label>
                            <div className={styles.selectPagoWrap}>
                                <ion-icon name="wallet-outline" />
                                <select
                                    id="metodo-pago-rapida"
                                    value={metodoPago}
                                    onChange={e => {
                                        setMetodoPago(e.target.value)
                                        if (e.target.value !== 'efectivo') setEfectivoRecibido('')
                                    }}
                        ...
                        <div className={styles.campoCompacto}>
                            ...
                            <button
                                type="button"
                                className={styles.btnAjustarPrecio}
            ...
            {productoPrecioEditando && (
                <div className={styles.modalBg} onClick={() => setProductoPrecioEditando(null)}>
                    <div className={styles.modal} onClick={e => e.stopPropagation()}>
                        <div className={styles.modalHead}>
                            <h3>Ajustar precio</h3>
                            <button type="button" onClick={() => setProductoPrecioEditando(null)}>
                                <ion-icon name="close-outline" />
                            </button>
                        </div>
                        <div className={styles.modalBody}>
                            {carrito.length > 1 && (
                                <div className={styles.grupoInput}>
                                    ...
                                    <select
                                        value={productoPrecioEditando.id}
                                        onChange={e => {
                                            const producto = carrito.find(item => String(item.id) === e.target.value)
                                            if (producto) abrirAjustePrecio(producto)
                                        }}
                                        className={styles.inputField}
                                    >
                                        {carrito.map(item => <option key={item.id} value={item.id}>{item.nombre}</option>)}
                                    </select>
                                </div>
                            )}
                            <div className={styles.infoChip}>
                                <ion-icon name="cube-outline" />
                                <div><label>Producto</label><p>{productoPrecioEditando.nombre}</p></div>
                            </div>
                            <div className={styles.grupoInput}>
                                <label>Nuevo precio unitario</label>
                                <div className={styles.inputConIcono}>
                                    <span className={styles.iconoMoneda}>{empresa?.simbolo_moneda || 'RD$'}</span>
                                    <input type="number" min="0.01" step="0.01" value={precioTemporal} onChange={e => setPrecioTemporal(e.target.value)} className={styles.inputMoneda} autoFocus />
                                </div>
                            </div>
                        </div>
                        <div className={styles.modalFoot}>
                            <button type="button" className={styles.cancelBtn} onClick={restablecerPrecioCatalogo}>Restablecer</button>
                            <button type="button" className={styles.confirmBtn} onClick={aplicarPrecioAjustado}>Aplicar precio</button>
                        </div>
                    </div>
                </div>
            )}
