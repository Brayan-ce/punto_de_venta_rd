...
import { obtenerDatosCaja, abrirCaja, cerrarCaja, obtenerVentas, anularVenta, obtenerDatosEmpresa, obtenerVentasDiaCaja, obtenerCajasEmpresa } from './servidor'
...
export default function VentasAdmin({ basePath = '/admin' }) {
    ...
    // 🎯 Filtros principales (por defecto: HOY + caja actual, mismo día y misma caja)
    const [periodo, setPeriodo] = useState('hoy') // 'hoy', 'semana', 'mes', 'personalizado'
    const [filtroCaja, setFiltroCaja] = useState('abierta') // 'abierta' | 'todas' | <cajaId>
    ...
    useEffect(() => {
        ...
        cargarVentas(1)
    }, [periodo, filtroCaja, filtrosAvanzados, fechaInicio, fechaFin])
    ...
    const verificarCaja = async () => {
        try {
            ...
            const dia = await obtenerVentasDiaCaja()
            if (dia.success) setVentasDia(dia.ventasDia || 0)
            const listaCajas = await obtenerCajasEmpresa()
            if (listaCajas.success) setCajas(listaCajas.cajas || [])
        } catch (error) {
    ...
    const manejarAnularVenta = async (ventaId, numeroInterno) => {
        ...
        try {
            ...
            if (resultado.success) {
                await cargarVentas()
                const creditoLiberado = Number(resultado.creditoLiberado || 0)
                Swal.fire(
                    t('ventas.anuladaExito'),
                    creditoLiberado > 0
                        ? `${resultado.mensaje}. ${tr('Crédito liberado:', 'Credit released:')} ${formatearMoneda(creditoLiberado)}`
                        : resultado.mensaje,
                    'success'
                )
            } else {
    ...
    const textoFiltroCaja = filtroCaja === 'abierta'
        ? (datosCaja ? `${t('ventas.cajaAbiertaOpcion')} #${datosCaja.numero_caja}` : t('ventas.cajaAbiertaOpcion'))
        : filtroCaja === 'todas'
            ? t('ventas.todasLasCajas')
            : `#${cajas.find(c => String(c.id) === String(filtroCaja))?.numero_caja ?? filtroCaja}`
    ...
    return (
        <div className={`${estilos.contenedorOptimizado} ${estilos[tema]}`}>
                    ...
                    <p className={estilos.subtitulo}>
                        {filtroCaja === 'abierta'
                            ...
                            : filtroCaja === 'todas'
                                ...
                                : `${t('ventas.mostrandoCaja')} #${cajas.find(c => String(c.id) === String(filtroCaja))?.numero_caja ?? filtroCaja}`}
                    </p>
                </div>
            ...
            {/* ========== ESTADO DE CAJA ========== */}
            <div className={`${estilos.cajaPanel} ${cajaAbierta && datosCaja ? estilos.cajaPanelAbierta : estilos.cajaPanelCerrada}`}>
                <div className={estilos.cajaPanelEstado}>
                    <div className={estilos.cajaPanelIcono}>
                        <ion-icon name={cajaAbierta && datosCaja ? 'cash-outline' : 'lock-closed-outline'}></ion-icon>
                    </div>
                    <div className={estilos.cajaPanelEstadoTexto}>
                        <span className={estilos.cajaPanelEtiqueta}>{tr('Estado de caja', 'Register status')}</span>
                        <span className={estilos.cajaPanelTitulo}>
                            {cajaAbierta && datosCaja
                                ? `${tr('Caja abierta', 'Register open')} #${datosCaja.numero_caja}`
                                : tr('Caja cerrada', 'Register closed')}
                        </span>
                    </div>
                </div>

                <div className={estilos.cajaPanelDatos}>
                    {cajaAbierta && datosCaja ? (
                        <>
                            <div className={estilos.cajaMetrica}>
                                <span className={estilos.cajaMetricaLabel}>{t('ventas.montoInicial')}</span>
                                <span className={estilos.cajaMetricaValor}>{formatearMoneda(datosCaja.monto_inicial)}</span>
                            </div>
                            <div className={estilos.cajaMetrica}>
                                <span className={estilos.cajaMetricaLabel}>{t('ventas.ventasDelDia')}</span>
                                <span className={estilos.cajaMetricaValor}>{formatearMoneda(ventasDia)}</span>
                            </div>
                        </>
                    ) : (
                        <p className={estilos.cajaPanelHint}>
                            {tr('Abre una caja para registrar y consultar las ventas del turno actual.', 'Open a register to record and view the current shift sales.')}
                        </p>
                    )}
                </div>

                <div className={estilos.cajaPanelAccion}>
                    {!cajaAbierta ? (
                        <button
                            className={`${estilos.btnCaja} ${estilos.btnCajaAbrir}`}
                        ...
                        <button
                            className={`${estilos.btnCaja} ${estilos.btnCajaCerrar}`}
            ...
            <div className={estilos.filtrosRapidos}>
                <div className={estilos.filtrosGrupo}>
                    <span className={estilos.filtrosGrupoLabel}>{t('ventas.periodo')}</span>
                ...
                <div className={estilos.filtrosGrupo}>
                    <span className={estilos.filtrosGrupoLabel}>{t('ventas.metodoPago')}</span>
                ...
                <div className={`${estilos.filtrosGrupo} ${estilos.filtrosGrupoCaja}`}>
                    <label className={estilos.filtrosGrupoLabel} htmlFor="filtroCajaVentas">
                        <ion-icon name="cash-outline"></ion-icon>
                        {tr('Caja a consultar', 'Register to view')}
                    </label>
                    <select
                        id="filtroCajaVentas"
                        value={filtroCaja}
                    ...
                    >
                        {cajaAbierta && datosCaja && (
                            <option value="abierta">{t('ventas.cajaAbiertaOpcion')} #{datosCaja.numero_caja}</option>
                        )}
                        <option value="todas">{t('ventas.todasLasCajas')}</option>
                        {cajas
                            .filter(c => !(cajaAbierta && datosCaja && String(c.id) === String(datosCaja.id)))
                            .map(c => (
                                <option key={c.id} value={String(c.id)}>
                                    #{c.numero_caja} · {c.estado === 'abierta'
                                        ? t('ventas.cajaAbiertaOpcion')
                                        : (c.fecha_apertura ? formatearFechaVisual(String(c.fecha_apertura).slice(0, 10)) : c.estado)}
                                </option>
                            ))}
                    </select>
                </div>

                <div className={estilos.filtrosResumen}>
                    <span className={estilos.filtrosResumenItem}>
                        <ion-icon name="receipt-outline"></ion-icon>
                        {totalVentas} {t('ventas.resultadosVentas')}
                    </span>
                    <span className={estilos.filtrosResumenSep}>•</span>
                    <span className={estilos.filtrosResumenItem}>
