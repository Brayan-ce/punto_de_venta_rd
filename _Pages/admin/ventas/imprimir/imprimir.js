...
export default function ImprimirVenta({ returnPath = '/admin/ventas' }) {
    ...
    const manejarImprimirNavegador = () => {
        document.body.classList.add('modoImpresionFactura')
        window.addEventListener('afterprint', () => {
            document.body.classList.remove('modoImpresionFactura')
        }, { once: true })
        window.print()
    }
    ...
    const manejarImprimirTermica = async () => {
        ...
        try {
            ...
            const ticketData = {
                empresa: {
                    nombre_empresa: empresa.nombre_empresa || empresa.razon_social,
                ...
                tipo_comprobante_nombre: traducirTipoComprobante(venta.tipo_comprobante_nombre),
                ncf: venta.ncf || 'N/A',
                numero_interno: venta.numero_interno,
                fecha: new Date(venta.fecha_venta).toLocaleString(language === 'en' ? 'en-US' : 'es-DO'),
                vendedor: venta.usuario_nombre,
                cliente_nombre: venta.cliente_nombre,
                cliente_direccion: venta.cliente_direccion || venta.direccion_cliente,
                productos: venta.productos,
                subtotal: parseFloat(venta.subtotal),
                descuento: parseFloat(venta.descuento || venta.descuento_total || 0),
                itbis: parseFloat(venta.itbis || 0),
                total: parseFloat(venta.total),
                metodoPago: traducirMetodoPago(venta.metodo_pago, venta.metodo_pago_texto),
                recibido: parseFloat(venta.monto_recibido || venta.total),
                cambio: parseFloat(venta.devuelta || 0),
                mensaje: tr('¡Gracias por su compra!', 'Thank you for your purchase!')
            }

            const configResponse = await fetch('/api/printer/config')
            const printerConfig = configResponse.ok ? (await configResponse.json()).config : null

            const usarRed = printerConfig?.connection === 'network' && printerConfig?.ip
            ...
            if (usarRed) {
                ...
                if (!response.ok || !result.success) throw new Error(result.error || 'La impresora LAN no respondió')
            } else {
                if (!impresoraSeleccionada) {
                    throw new Error(tr('Por favor selecciona una impresora', 'Please select a printer'))
                }
    ...
    const formatearCantidad = (cantidad, maximos = 3) => {
    ...
    const generarTextoComprobante = () => {
        ...
        venta.productos.forEach(p => {
            ...
            const cantidadFormateada = formatearCantidad(p.cantidad)
            
            let cantidadTexto = `${cantidadFormateada} ${unidadVenta}`
            if (hayConversion && p.cantidad_base) {
                const cantidadBaseFormateada = formatearCantidad(p.cantidad_base, 2)
                            ...
                            {venta.productos.map((producto, index) => {
                                ...
                                const formatearCantidadConUnidad = () => {
                                    ...
                                    if (esDespachoParcial) {
                                        const cantidadDespachadaFormateada = formatearCantidad(producto.cantidad_despachada)
                                        if (hayConversion && producto.cantidad_base) {
                                            const cantidadBaseFormateada = formatearCantidad(producto.cantidad_base, 2)
                                            return (
                                                <span>
                                        ...
                                        <td>
                                            ...
                                            {esDespachoParcial && (
                                                <div style={{ fontSize: '0.85em', color: '#ef4444', marginTop: '2px' }}>
                                                    {tr('Pendiente', 'Pending')}: {formatearCantidad(cantidadPendiente)} {unidadVenta}
                                                </div>
                                            )}
                                        </td>
                                ...
                                <table className={estilos.productos}>
                                    ...
                                    <tbody>
                                        {venta.extras.map((extra, index) => (
                                            <tr key={index}>
                                                <td className={estilos.centrado}>{formatearCantidad(extra.cantidad)}</td>
                                                <td>
                                                    {extra.nombre}
                                                    {extra.tipo && (
