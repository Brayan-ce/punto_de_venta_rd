    ...
    try {
        ...
        const [productos] = await connection.execute(
            `SELECT 
                ...
                COALESCE(pc.visible_catalogo, TRUE) as visible_catalogo,
                pc.precio_catalogo,
                pc.precio_oferta as precio_oferta_catalogo,
                pc.fecha_inicio_oferta,
                pc.fecha_fin_oferta,
                COALESCE(pc.destacado, FALSE) as destacado,
                pc.orden_visual,
                pc.descripcion_corta,
                COALESCE(pc.stock_visible, TRUE) as stock_visible,
                COALESCE(pc.activo, TRUE) as activo_catalogo
            FROM productos p
    ...
    try {
        ...
        if (existentes.length > 0) {
            ...
            await connection.execute(
                ...
                WHERE producto_id = ? AND empresa_id = ?`,
                [
                    datos.visible_catalogo !== undefined ? datos.visible_catalogo : actual.visible_catalogo,
                    datos.precio_catalogo !== undefined ? datos.precio_catalogo : actual.precio_catalogo,
                    datos.precio_oferta !== undefined ? datos.precio_oferta : actual.precio_oferta,
                    datos.fecha_inicio_oferta !== undefined ? datos.fecha_inicio_oferta : actual.fecha_inicio_oferta,
                    datos.fecha_fin_oferta !== undefined ? datos.fecha_fin_oferta : actual.fecha_fin_oferta,
                    datos.destacado !== undefined ? datos.destacado : actual.destacado,
                    datos.orden_visual !== undefined ? datos.orden_visual : actual.orden_visual,
                    datos.descripcion_corta !== undefined ? datos.descripcion_corta : actual.descripcion_corta,
                    datos.stock_visible !== undefined ? datos.stock_visible : actual.stock_visible,
                    datos.activo !== undefined ? datos.activo : actual.activo,
                    productoId,
                    empresaId
                ]
            )
        ...
        if (existentes.length > 0) {
            await connection.execute(
                `UPDATE productos_catalogo
                 SET visible_catalogo = ?, activo = ?
                 WHERE producto_id = ? AND empresa_id = ?`,
                [visible, visible, productoId, empresaId]
            )
        } else {
            ...
            await connection.execute(
                ...
                VALUES (?, ?, ?, ?)`,
                [productoId, empresaId, visible, visible]
            )
        }
