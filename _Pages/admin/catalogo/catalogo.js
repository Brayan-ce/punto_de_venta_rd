...
export default function CatalogoAdmin() {
    ...
    const generarCodigoQr = async () => {
        if (!config?.url_slug || generandoQr) return

        setGenerandoQr(true)
        try {
            const catalogUrl = `${baseUrl}/catalogo/${config.url_slug}`
            const imagenQr = await QRCode.toDataURL(catalogUrl, {
                width: 1024,
                margin: 2,
            ...
            enlace.download = `catalogo-${config.url_slug}.png`
            document.body.appendChild(enlace)
            enlace.click()
            document.body.removeChild(enlace)
        } catch (error) {
            console.error('Error al generar el código QR:', error)
            alert(tr('No se pudo generar el código QR', 'The QR code could not be generated'))
        } finally {
                        ...
                        <div className={estilos.sidebarCardCompartir}>
                            ...
                            <div className={estilos.compartirAcciones}>
                                ...
                                >
                                    ...
                                    <span>{generandoQr ? tr('Generando...', 'Generating...') : tr('Generar Código QR', 'Generate QR Code')}</span>
                                </button>
                                <button
