...
import { imprimirSelloZebra } from '@/utils/sellosZebra'
...
export default function ProductosAdmin() {
    ...
    const sellosZebraActivo = tieneModulo('sellos_zebra')
    ...
    const manejarImprimirSello = async (producto) => {
        try {
            await imprimirSelloZebra(producto, '')
        } catch (error) {
            alert(error.message || tr('No se pudo imprimir el sello', 'Could not print label'))
        }
    }
        ...
        if (vistaActual === 'tabla') {
            return (
                <TablaProductosMemo
                    ...
                    sellosZebraActivo={sellosZebraActivo}
                    manejarImprimirSello={manejarImprimirSello}
                />
            )
        }
            ...
            <div className={estilos.controles}>
                ...
                <div className={estilos.filtros}>
                    ...
                    >
                        ...
                        <option value="por_vencer">{tr('Por vencer (30 dias)', 'Expiring soon (30 days)')}</option>
                        <option value="vencido">{tr('Vencidos', 'Expired')}</option>
                    </select>
...
// Determina si un producto esta vencido o proximo a vencer (30 dias)
...
// ===============================================================
const ProductoCard = React.memo(function ProductoCard({ producto, tema, formatearMoneda, manejarEliminar, procesando, estilos, t, tr, seleccionado, toggleSeleccion, sellosZebraActivo, manejarImprimirSello }) {
    return (
        <div className={`${estilos.card} ${estilos[tema]}`}>
            ...
            <div className={estilos.cardHeader}>
                ...
                {(() => {
                    const vto = estadoVencimiento(producto.fecha_vencimiento)
                    if (!vto) return null
                    return (
                        <span className={`${estilos.badgeBajoStock} ${vto.tipo === 'proximo' ? estilos.badgeProximoVencer : ''}`} style={{ top: producto.stock <= producto.stock_minimo ? 44 : 12 }}>
                            {vto.tipo === 'vencido'
                                ? tr('Vencido', 'Expired')
                                : tr(`Vence en ${vto.dias}d`, `Expires in ${vto.dias}d`)}
                        </span>
                    )
                })()}
            </div>
            ...
            <div className={estilos.cardBody}>
                ...
                {producto.ubicacion_bodega && (
                    <span className={estilos.ubicacionBodega}>
                        <ion-icon name="location-outline"></ion-icon> {producto.ubicacion_bodega}
                    </span>
                )}
            ...
            <div className={estilos.cardFooter}>
                {sellosZebraActivo && (
                    <button onClick={() => manejarImprimirSello(producto)} className={estilos.btnIcono} title={tr('Imprimir sello Zebra', 'Print Zebra label')}>
...
// ===============================================================
const TablaProductos = React.memo(function TablaProductos({ productos, tema, router, formatearMoneda, manejarEliminar, procesando, estilos, t, tr, seleccionados, toggleSeleccion, toggleSeleccionarTodo, sellosZebraActivo, manejarImprimirSello }) {
            ...
            <table className={estilos.tabla}>
                <thead>
                    <tr className={estilos[tema]}>
                        ...
                        {sellosZebraActivo && <th>{tr('Sello', 'Label')}</th>}
                        <th>{t('pages.stock')}</th>
                        <th>{tr('Vencimiento/Bodega', 'Expiry/Location')}</th>
                ...
                <tbody>
                    {productos.map((producto) => (
                        <tr key={producto.id} className={`${estilos.filaTabla} ${estilos[tema]}`}>
                            ...
                            <td>
                                {sellosZebraActivo && (
                                    <button type="button" className={estilos.btnIcono} onClick={() => manejarImprimirSello(producto)} title={tr('Imprimir sello Zebra', 'Print Zebra label')}>
                                        <ion-icon name="barcode-outline"></ion-icon>
                                    </button>
                                )}
                            </td>
                            ...
                            <td>
                                {(() => {
                                    const vto = estadoVencimiento(producto.fecha_vencimiento)
                                    return (
                                        <div className={estilos.vencimientoTabla}>
                                            {vto && (
                                                <span className={`${estilos.badgeBajoStockTabla} ${vto.tipo === 'proximo' ? estilos.badgeProximoVencerTabla : ''}`}>
                                                    {vto.tipo === 'vencido'
                                                        ? tr('Vencido', 'Expired')
                                                        : tr(`Vence en ${vto.dias}d`, `Expires in ${vto.dias}d`)}
                                                </span>
                                            ...
                                            {producto.ubicacion_bodega && (
                                                <span className={estilos.ubicacionBodegaTabla}>
                                                    <ion-icon name="location-outline"></ion-icon> {producto.ubicacion_bodega}
                                                </span>
                                            )}
                                            {!vto && !producto.ubicacion_bodega && '—'}
                                        </div>
                                    )
                                })()}
                            </td>
