...
export default function ImportarProductos({ onImportarCompleto }) {
    ...
    const manejarSeleccionArchivo = (e) => {
        ...
        if (file) {
            ...
            if (!fileName.endsWith('.xlsx') && !fileName.endsWith('.xls')) {
                alert(tr('Por favor selecciona un archivo Excel (.xlsx o .xls)', 'Please select an Excel file (.xlsx or .xls)'))
                return
            }
            
            // Validar tamaño (50MB para storage temporal)
            if (file.size > 50 * 1024 * 1024) {
                alert(tr('El archivo es demasiado grande (máximo 50MB). Por favor, divide el archivo en partes más pequeñas o comprime el Excel.', 'The file is too large (maximum 50MB). Please split the file into smaller parts or compress the Excel file.'))
                return
            }
    ...
    const manejarSubirArchivo = async () => {
        if (!archivo) {
            alert(tr('Por favor selecciona un archivo Excel', 'Please select an Excel file'))
            return
        }
        ...
        try {
            ...
            setResultado({
                success: false,
                            mensaje: tr(`Error al subir el archivo: ${error.message}. Intenta comprimir el Excel o dividirlo manualmente.`, `Error uploading file: ${error.message}. Try compressing the Excel file or splitting it manually.`),
                estadisticas: {
                    total: 0,
                    procesados: 0,
                    creados: 0,
    ...
    const subirArchivoCompleto = async (file) => {
        ...
        xhr.onload = async () => {
            try {
                ...
                if (!contentType || !contentType.includes('application/json')) {
                    if (xhr.status === 413) {
                        ...
                        if (file.size > 10 * 1024 * 1024) {
                            setResultado({
                                success: false,
                                mensaje: tr('El archivo es demasiado grande. El sistema intentará dividirlo automáticamente. Por favor, intenta de nuevo.', 'The file is too large. The system will try to split it automatically. Please try again.'),
                                estadisticas: {
                                    total: 0,
                                    procesados: 0,
                                    creados: 0,
                        ...
                        setResultado({
                            success: false,
                            mensaje: tr('El archivo es demasiado grande. Por favor, comprime el Excel o divídelo manualmente.', 'The file is too large. Please compress the Excel file or split it manually.'),
                            estadisticas: {
                                total: 0,
                                procesados: 0,
                                creados: 0,
                ...
                setResultado({
                    success: false,
                    mensaje: tr(`Error al subir el archivo: ${error.message}`, `Error uploading file: ${error.message}`)
                })
                setSubiendo(false)
            }
        }
        ...
        xhr.onerror = () => {
            ...
            setResultado({
                success: false,
                mensaje: tr('Error de red al subir el archivo. Verifica tu conexión.', 'Network error uploading file. Check your connection.')
            })
            setSubiendo(false)
            xhrRef.current = null
        }
        ...
        xhr.ontimeout = () => {
            ...
            setResultado({
                success: false,
                mensaje: tr('Timeout al subir el archivo. El archivo puede ser muy grande.', 'Upload timeout. The file may be too large.')
            })
            setSubiendo(false)
            xhrRef.current = null
        }
    ...
    const manejarProcesar = async (fileIdToProcess) => {
        ...
        if (!targetFileId) {
            alert(tr('No hay archivo para procesar', 'No file to process'))
            return
        }
        ...
        try {
            ...
            setResultado({
                success: false,
                mensaje: tr(`Error al iniciar el procesamiento: ${error.message}`, `Error starting processing: ${error.message}`)
            })
            setProcesando(false)
        }
    }
                ...
                if (data.success) {
                    ...
                    if (data.estado === 'completed' || data.estado === 'failed') {
                        ...
                        setResultado({
                            ...
                            mensaje: data.mensaje || (data.estado === 'completed' ? tr('Importación completada exitosamente', 'Import completed successfully') : tr('Error en la importación', 'Import failed')),
    ...
    const cerrarModal = () => {
        ...
        if (subiendo || procesando) {
            if (confirm(tr('La importación está en progreso. ¿Deseas cancelar y cerrar el modal?', 'Import is in progress. Do you want to cancel and close the modal?'))) {
                cancelarImportacion()
            } else {
    ...
    return (
        <>
            ...
            <button
                ...
                title={tr('Importar productos desde Excel', 'Import products from Excel')}
            >
                <ion-icon name="cloud-upload-outline"></ion-icon>
                <span>{tr('Importar Excel', 'Import Excel')}</span>
            </button>
                ...
                <div className={estilos.modalOverlay} onClick={cerrarModal}>
                    <div className={estilos.modalContent} onClick={(e) => e.stopPropagation()}>
                        <div className={estilos.modalHeader}>
                            <h2 className={estilos.modalTitulo}>{tr('Importar Productos desde Excel', 'Import Products from Excel')}</h2>
                            <button
                                onClick={cerrarModal}
                            ...
                            {!resultado && currentStep === 'idle' ? (
                                    ...
                                    <div className={estilos.timeline}>
                                        ...
                                        <div className={`${estilos.timelineStep} ${estilos.completed}`}>
                                            ...
                                            <div className={estilos.timelineContent}>
                                                <p className={estilos.timelineTitle}>{tr('Archivo seleccionado', 'File selected')}</p>
                                        ...
                                        <div className={`${estilos.timelineStep} ${subiendo ? estilos.active : fileId ? estilos.completed : estilos.pending}`}>
                                            ...
                                            <div className={estilos.timelineContent}>
                                                <p className={estilos.timelineTitle}>
                                                    {subiendo ? tr('Subiendo archivo...', 'Uploading file...') : fileId ? tr('Archivo subido', 'File uploaded') : tr('Subiendo archivo', 'Uploading file')}
                                                </p>
                                                {subiendo && (
                                                    <>
                                                ...
                                                <p className={estilos.timelineTitle}>
                                                    {procesando 
                                                        ? (estadoJob?.mensaje?.includes('Guardando') 
                                                            ? tr('Guardando en base de datos...', 'Saving to database...')
                                                            : tr('Procesando productos...', 'Processing products...'))
                                                        : estadoJob?.estado === 'completed' 
                                                        ? tr('Importación completada', 'Import completed')
                                                        : tr('Procesando productos', 'Processing products')}
                                                </p>
                                                {procesando && estadoJob?.estadisticas && (
                                                    <>
                                                        ...
                                                        <p className={estilos.progresoTexto}>
                                                            {processingProgress}% - {formatearNumero(estadoJob.estadisticas.procesados || 0)} {tr('de', 'of')} {formatearNumero(estadoJob.estadisticas.total || 0)} {tr('productos', 'products')}
                                                        </p>
                                                        {estadoJob.estadisticas.procesados > 0 && (
                                                            <p className={estilos.progresoDetalle}>
                                                                ✓ {formatearNumero(estadoJob.estadisticas.creados || 0)} {tr('creados', 'created')}
                                                                {estadoJob.estadisticas.actualizados > 0 && (
                                                                    <> • ✓ {formatearNumero(estadoJob.estadisticas.actualizados)} {tr('actualizados', 'updated')}</>
                                                                )}
                                                                {estadoJob.estadisticas.total > estadoJob.estadisticas.procesados && (
                                                                    <> • ⏳ {formatearNumero(estadoJob.estadisticas.total - estadoJob.estadisticas.procesados)} {tr('restantes', 'remaining')}</>
                                                                )}
                                                            </p>
                                                        )}
                                                    </>
                                ...
                                <div className={estilos.resultadoImportacion}>
                                    ...
                                    <h3 className={estilos.resultadoTitulo}>
                                        {resultado.success ? tr('Importación Exitosa', 'Import Successful') : tr('Error en la Importación', 'Import Error')}
                                    </h3>
                            ...
                            {!resultado && (
                                <>
                                    <button
                                        onClick={() => {
                                            if (subiendo || procesando) {
                                                if (confirm(tr('¿Deseas cancelar la importación?', 'Do you want to cancel the import?'))) {
                                                    cancelarImportacion()
                                                }
                                            } else {
                                                cerrarModal()
                                        ...
                                        disabled={false}
                                    >
                                        {subiendo || procesando ? tr('Cancelar', 'Cancel') : tr('Cerrar', 'Close')}
                                    </button>
                                    <button
                                    ...
                                    >
                                        {subiendo ? (
                                            ...
                                            <>
                                                <ion-icon name="cloud-upload-outline"></ion-icon>
                                                {tr('Subir y Procesar', 'Upload and Process')}
                                            </>
                                        )}
                                    </button>
                                </>
