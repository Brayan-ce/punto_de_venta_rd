...
export async function obtenerProductos(params = {}) {
    ...
    try {
        ...
        if (estado === 'activo') {
            ...
            whereConditions.push("p.fecha_vencimiento IS NOT NULL AND p.fecha_vencimiento >= CURDATE() AND p.fecha_vencimiento <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)")
        } else if (estado === 'vencido') {
            whereConditions.push('p.fecha_vencimiento IS NOT NULL AND p.fecha_vencimiento < CURDATE()')
        }
