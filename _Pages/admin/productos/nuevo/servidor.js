...
async function asegurarUnidadesBaseEmpresa(connection, empresaId) {
    const [existentes] = await connection.execute(
        `SELECT id FROM unidades_medida WHERE empresa_id = ? LIMIT 1`,
        [empresaId]
    )
 ...
 */
async function autoConfigurarProducto(producto, connection, empresaId) {
    ...
    try {
        const [unidad] = await connection.execute(
            ...
            [producto.unidad_medida_id, empresaId]
        )
        
        if (unidad.length === 0) return null
...
export async function obtenerDatosProducto() {
    ...
    try {
        ...
        await asegurarUnidadesBaseEmpresa(connection, empresaId)
...
export async function crearProducto(datosProducto) {
    ...
    try {
        ...
        const precioCompra = Number(datosProducto.precio_compra)
        const precioVenta = Number(datosProducto.precio_venta)
        const precioPorUnidad = Number(datosProducto.precio_por_unidad || datosProducto.precio_venta)
        const stockMinimo = Number(datosProducto.stock_minimo)
        const stockMaximo = Number(datosProducto.stock_maximo)

        if (!datosProducto.nombre?.trim()) {
            return { success: false, mensaje: 'El nombre del producto es obligatorio' }
        }

        if (!Number.isFinite(precioCompra) || precioCompra < 0 || !Number.isFinite(precioVenta) || precioVenta <= 0 || !Number.isFinite(precioPorUnidad) || precioPorUnidad <= 0) {
            return { success: false, mensaje: 'Verifica los precios del producto' }
        }

        if (!datosProducto.unidad_medida_id) {
            return { success: false, mensaje: 'Selecciona una unidad de medida válida' }
        }

        if (!Number.isFinite(stockMinimo) || !Number.isFinite(stockMaximo) || stockMinimo < 0 || stockMaximo < stockMinimo) {
            return { success: false, mensaje: 'El stock máximo debe ser mayor o igual al stock mínimo' }
        }
        ...
        datosProducto = await autoConfigurarProducto(datosProducto, connection, empresaId)

        if (!datosProducto) {
            await connection.rollback()
            connection.release()
            return { success: false, mensaje: 'La unidad de medida seleccionada no es válida para esta empresa' }
        }
        ...
        if (Array.isArray(datosProducto.imagenes_catalogo_base64) && datosProducto.imagenes_catalogo_base64.length > 0) {
            const productoId = resultado.insertId
            for (const [orden, imagenBase64] of datosProducto.imagenes_catalogo_base64.entries()) {
                const imagenUrl = await guardarImagenProducto(imagenBase64, `${productoId}_catalogo_${orden + 1}`)
                await connection.execute(
                    `INSERT INTO producto_imagenes (producto_id, empresa_id, imagen_url, orden)
                     VALUES (?, ?, ?, ?)`,
                    [productoId, empresaId, imagenUrl, orden + 1]
                )
            }
        }
