...
export default function NuevoProductoAdmin({ returnPath = '/admin/productos' }) {
    ...
    const [imagenesCatalogo, setImagenesCatalogo] = useState([])
    const [recortandoImagen, setRecortandoImagen] = useState(false)
    ...
    const consultarCodigoBarras = async () => {
        ...
        if (!/^\d{8,14}$/.test(codigo)) {
            alert(tr('Introduce un código de barras numérico de 8 a 14 dígitos', 'Enter an 8 to 14 digit numeric barcode'))
            return
        }

        setConsultandoCodigo(true)
        try {
            const respuesta = await fetch(`/api/productos/codigo-barras?codigo=${encodeURIComponent(codigo)}`, { cache: 'no-store' })
            const resultado = await respuesta.json()
            if (!resultado.encontrado) {
                alert(resultado.mensaje || tr('No se encontró información para ese código', 'No information was found for this barcode'))
                return
            }
            ...
            if (producto.nombre) {
                ...
                setSku(prev => prev || producto.nombre.substring(0, 3).toUpperCase().replace(/\s/g, '') + '-' + (Math.floor(Math.random() * 9000) + 1000))
            }
            if (producto.descripcion) setDescripcion(prev => prev || producto.descripcion)
            if (producto.imagen_url && !imagenUrl && tipoImagen === 'url') {
                setImagenUrl(producto.imagen_url)
                setVistaPrevia(producto.imagen_url)
            }
            alert(tr('Información del producto cargada. Revisa los datos antes de guardar.', 'Product information loaded. Review it before saving.'))
        } catch (error) {
            console.error('Error consultando código de barras:', error)
            alert(tr('No se pudo consultar la información del código', 'Could not query barcode information'))
        } finally {
    ...
    const convertirRecorteABase64 = async () => {
        if (!vistaPrevia || !areaRecorte) return
        const imagen = new Image()
        imagen.src = vistaPrevia
        await new Promise((resolve, reject) => { imagen.onload = resolve; imagen.onerror = reject })
        const canvas = document.createElement('canvas')
        canvas.width = areaRecorte.width
        canvas.height = areaRecorte.height
        const contexto = canvas.getContext('2d')
        contexto.drawImage(imagen, areaRecorte.x, areaRecorte.y, areaRecorte.width, areaRecorte.height, 0, 0, areaRecorte.width, areaRecorte.height)
        setVistaPrevia(canvas.toDataURL('image/jpeg', 0.9))
        setRecortandoImagen(false)
    }

    const manejarImagenesCatalogo = (e) => {
        const archivos = Array.from(e.target.files || []).slice(0, 8 - imagenesCatalogo.length)
        for (const archivo of archivos) {
            if (!archivo.type.startsWith('image/') || archivo.size > 5 * 1024 * 1024) continue
            const reader = new FileReader()
            reader.onloadend = () => setImagenesCatalogo(actuales => [...actuales, reader.result].slice(0, 8))
            reader.readAsDataURL(archivo)
        }
        e.target.value = ''
    }
    ...
    const manejarSubmit = async (e) => {
        ...
        try {
            const datosProducto = {
                ...
                imagenes_catalogo_base64: imagenesCatalogo,
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}>
                ...
                <div className={estilos.layoutPrincipal}>
                        ...
                        <div className={`${estilos.seccion} ${estilos[tema]}`}>
                            ...
                            <div className={estilos.gridDosColumnas}>
                                <div className={estilos.grupoInput}>
                                    ...
                                    <div className={estilos.inputConAccion}>
                                        <input
                                            type="text"
                                            inputMode="numeric"
                                            value={codigoBarras}
                                        ...
                                        />
                                        <button type="button" className={estilos.btnConsultarCodigo} onClick={consultarCodigoBarras} disabled={consultandoCodigo}>
                                            <ion-icon name={consultandoCodigo ? 'sync-outline' : 'search-outline'}></ion-icon>
                                            <span>{consultandoCodigo ? tr('Buscando...', 'Searching...') : tr('Consultar', 'Lookup')}</span>
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className={estilos.columnaDerecha}>
                        <div className={`${estilos.seccion} ${estilos[tema]}`}>
                            ...
                            {tipoImagen === 'url' ? (
                                ...
                                <div className={estilos.grupoInput}>
                                    ...
                                    <div className={estilos.contenedorArchivo}>
                                        ...
                                        {vistaPrevia && (
                                            <button type="button" className={estilos.btnRecortar} onClick={() => setRecortandoImagen(true)}>
                                                <ion-icon name="crop-outline"></ion-icon>
                                                <span>{tr('Recortar imagen', 'Crop image')}</span>
                                            </button>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className={estilos.galeriaCarga}>
                                <label>{tr('Galería para catálogo online', 'Online catalog gallery')}</label>
                                <p>{tr('Añade hasta 8 imágenes para mostrar otros ángulos. No aparecen en el punto de venta.', 'Add up to 8 images for other angles. They do not appear in the point of sale.')}</p>
                                <input id="imagenes-catalogo" type="file" accept="image/*" multiple onChange={manejarImagenesCatalogo} className={estilos.inputFile} disabled={imagenesCatalogo.length >= 8} />
                                <label htmlFor="imagenes-catalogo" className={estilos.labelArchivo}>
                                    <ion-icon name="images-outline"></ion-icon>
                                    <span>{tr('Añadir imágenes', 'Add images')} ({imagenesCatalogo.length}/8)</span>
                                </label>
                                {imagenesCatalogo.length > 0 && (
                                    <div className={estilos.galeriaMiniaturas}>
                                        {imagenesCatalogo.map((imagen, indice) => (
                                            <div key={imagen} className={estilos.miniaturaGaleria}>
                                                <img src={imagen} alt="" />
                                                <button type="button" onClick={() => setImagenesCatalogo(actuales => actuales.filter((_, i) => i !== indice))} aria-label={tr('Eliminar imagen', 'Remove image')}><ion-icon name="close-outline"></ion-icon></button>
                                            </div>
                                        ))}
                                    </div>
                                )}
            ...
            {recortandoImagen && vistaPrevia && (
                <div className={estilos.modalRecorte} role="dialog" aria-modal="true" aria-label={tr('Recortar imagen', 'Crop image')}>
                    <div className={estilos.panelRecorte}>
                        <div className={estilos.areaRecorte}><Cropper image={vistaPrevia} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={(_, pixels) => setAreaRecorte(pixels)} /></div>
                        <input type="range" min="1" max="3" step="0.1" value={zoom} onChange={(e) => setZoom(Number(e.target.value))} aria-label={tr('Zoom de imagen', 'Image zoom')} />
                        <div className={estilos.accionesRecorte}>
                            <button type="button" className={estilos.btnCancelarForm} onClick={() => setRecortandoImagen(false)}>{tr('Cancelar', 'Cancel')}</button>
                            <button type="button" className={estilos.btnGuardar} onClick={convertirRecorteABase64}>{tr('Aplicar recorte', 'Apply crop')}</button>
                        </div>
                    </div>
                </div>
            )}
