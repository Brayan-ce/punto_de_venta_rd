...
export default function CajaPageAdmin() {
    ...
    const manejarCerrarCaja = async (e) => {
        ...
        try {
            ...
            if (resultado.success) {
                setReporteCierre({
                    numeroCaja: cajaActiva.numero_caja,
                    fechaApertura: cajaActiva.fecha_apertura,
                    fechaCierre: new Date().toISOString(),
                    montoInicial: parseFloat(cajaActiva.monto_inicial || 0),
                    ventas: parseFloat(cajaActiva.total_ventas || 0),
                    gastos: parseFloat(cajaActiva.total_gastos || 0),
                    esperado,
                    montoFinal: parseFloat(formCerrar.monto_final),
                    diferencia: resultado.diferencia ?? diferencia,
                    metodoPago: formCerrar.metodo_pago_cierre,
                    notas: formCerrar.notas.trim(),
                    empresa: empresa?.nombre || empresa?.nombre_empresa || 'ISIWeek'
                })
    ...
    const datosReporte = (reporte) => [
        [tr('Caja', 'Register'), reporte.numeroCaja],
        [tr('Apertura', 'Opening'), formatearFecha(reporte.fechaApertura)],
        [tr('Cierre', 'Closing'), formatearFecha(reporte.fechaCierre)],
        [tr('Monto inicial', 'Initial amount'), formatearMoneda(reporte.montoInicial)],
        [tr('Ventas', 'Sales'), formatearMoneda(reporte.ventas)],
        [tr('Gastos', 'Expenses'), formatearMoneda(reporte.gastos)],
        [tr('Esperado en caja', 'Expected in register'), formatearMoneda(reporte.esperado)],
        [tr('Monto contado', 'Counted amount'), formatearMoneda(reporte.montoFinal)],
        [tr('Diferencia', 'Difference'), formatearMoneda(reporte.diferencia)],
        [tr('Método de cierre', 'Closing method'), reporte.metodoPago],
        [tr('Notas', 'Notes'), reporte.notas || '-']
    ]

    const descargarExcel = () => {
        if (!reporteCierre) return
        const hoja = XLSX.utils.aoa_to_sheet([
            [tr('Cuadre de Caja', 'Cash Reconciliation')],
            [reporteCierre.empresa],
            [],
            ...datosReporte(reporteCierre)
        ])
        hoja['!cols'] = [{ wch: 24 }, { wch: 30 }]
        const libro = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(libro, hoja, tr('Cuadre', 'Reconciliation'))
        XLSX.writeFile(libro, `cuadre-caja-${reporteCierre.numeroCaja}.xlsx`)
    }
    ...
    const descargarPdf = () => {
        ...
        const pdf = new jsPDF()
        pdf.setFontSize(18)
        pdf.text(tr('Cuadre de Caja', 'Cash Reconciliation'), 20, 20)
        pdf.setFontSize(12)
        pdf.text(reporteCierre.empresa, 20, 29)
        let posicionY = 42
        datosReporte(reporteCierre).forEach(([etiqueta, valor]) => {
            pdf.text(`${etiqueta}:`, 20, posicionY)
            pdf.text(String(valor), 90, posicionY)
            posicionY += 9
        })
        pdf.save(`cuadre-caja-${reporteCierre.numeroCaja}.pdf`)
    }

    const imprimirTicket = () => {
        if (!reporteCierre) return
        const escaparHtml = (valor) => String(valor).replace(/[&<>'"]/g, caracter => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[caracter])
        const filas = datosReporte(reporteCierre)
            .map(([etiqueta, valor]) => `<tr><td>${escaparHtml(etiqueta)}</td><td>${escaparHtml(valor)}</td></tr>`)
            .join('')
    ...
    return (
        <div className={`${estilos.contenedor} ${estilos[tema]}`}> 
            ...
            {reporteCierre && (
                <div className={estilos.modal}>
                    <div className={`${estilos.modalContenido} ${estilos[tema]} ${estilos.modalReporte}`}>
                        <div className={estilos.modalHeader}>
                            <h3>{tr('Caja cerrada', 'Register closed')}</h3>
                            <button onClick={() => setReporteCierre(null)} className={estilos.btnCerrarModal}>
                                <ion-icon name="close-outline"></ion-icon>
                            </button>
                        </div>
                        <div className={estilos.modalBody}>
                            <div className={estilos.alertaInfo}>
                                ...
                                <span>{tr('El cuadre fue guardado correctamente. Elige cómo deseas conservar el comprobante.', 'The reconciliation was saved. Choose how you want to keep the receipt.')}</span>
                            </div>
                            <div className={estilos.resumenCierre}>
                                <div className={`${estilos.resumenItem} ${estilos.total}`}>
                                    <span>{tr('Diferencia de cierre', 'Closing difference')}</span>
                                    <span>{formatearMoneda(reporteCierre.diferencia)}</span>
                                </div>
                            </div>
                        </div>
                        <div className={estilos.accionesReporte}>
                            <button onClick={imprimirTicket} className={estilos.btnTicket}>
                                <ion-icon name="print-outline"></ion-icon><span>{tr('Imprimir ticket', 'Print ticket')}</span>
                            </button>
                            <button onClick={descargarPdf} className={estilos.btnPdf}>
                                <ion-icon name="document-text-outline"></ion-icon><span>{tr('Guardar PDF', 'Save PDF')}</span>
                            </button>
                            <button onClick={descargarExcel} className={estilos.btnExcel}>
                                <ion-icon name="grid-outline"></ion-icon><span>{tr('Guardar Excel', 'Save Excel')}</span>
                            </button>
                        </div>
                    </div>
                </div>
