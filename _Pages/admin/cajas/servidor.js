...
// Cambia aquí la zona horaria por defecto si lo necesitas
const ZONA_HORARIA_LOCAL = process.env.ZONA_HORARIA_LOCAL || 'America/Santo_Domingo'
...
const [cajas] = await connection.execute(
    `SELECT *, DATE_FORMAT(fecha_caja, '%Y-%m-%d') as fecha_caja
     FROM cajas 
     WHERE empresa_id = ? 
       AND usuario_id = ? 
       AND estado = 'abierta'
...
const [cajaActiva] = await connection.execute(
    ...
    [empresaId, userId]
        ...
        // Obtener fecha local en zona horaria definida
        const fechaHoy = dayjs().tz(ZONA_HORARIA_LOCAL).format('YYYY-MM-DD')

        if (cajaActiva.length > 0) {
            const fechaCaja = cajaActiva[0].fecha_caja
            // Comparar usando zona horaria local
            if (dayjs(fechaCaja).isBefore(dayjs(fechaHoy))) {
...
export async function obtenerHistorialCajas() {
    ...
    try {
        ...
        const [cajas] = await connection.execute(
            `SELECT c.id,
                    c.numero_caja,
                    DATE_FORMAT(c.fecha_caja, '%Y-%m-%d') as fecha_caja,
                    c.monto_inicial,
                    c.monto_final,
                    c.total_ventas,
                    c.total_efectivo,
