...
import { useLanguage } from '@/_Pages/vendedor/i18n'
...
export default function HeaderVendedor() {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                if (resultado.success) {
                    localStorage.setItem('cachedVendedorSession', JSON.stringify({
        ...
        const usarCacheLocal = () => {
            const cached = localStorage.getItem('cachedVendedorSession')
            if (cached) {
                ...
                localStorage.removeItem('cachedVendedorSession')
            }
            router.push('/login')
        }
        cargarDatos()
