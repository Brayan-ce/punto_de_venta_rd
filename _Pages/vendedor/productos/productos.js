...
import { obtenerDatosEmpresa } from '../dashboard/servidor'
import { 
    ...
    const cargarProductos = async () => {
        ...
        try {
            ...
            if (resultado.success) {
                ...
                setProductos(prods)
                const cats = [...new Map(prods.filter(p => p.categoria_id).map(p => [p.categoria_id, { id: p.categoria_id, nombre: p.categoria_nombre }])).values()]
                const marcasArr = [...new Map(prods.filter(p => p.marca_id).map(p => [p.marca_id, { id: p.marca_id, nombre: p.marca_nombre }])).values()]
                setCategorias(cats)
                setMarcas(marcasArr)
            } else {
