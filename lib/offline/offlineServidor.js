...
// Estas tablas pertenecen a módulos web que no forman parte del esquema local
// de la APK. Se mantienen en el servidor y no se incluyen en el paquete móvil.
const TABLAS_EXCLUIDAS_MOVIL = new Set([
  'bancos',
  'cuentas_bancarias',
...
export async function descargarDatosEmpresa(connection, userId, empresaId, opciones = {}) {
  ...
  try {
    ...
    for (const t of tablasConEmpresa) {
      if (TABLAS_EXCLUIDAS_MOVIL.has(t.tabla)) continue
      try {
    ...
    for (const h of tablasHijas) {
      if (TABLAS_EXCLUIDAS_MOVIL.has(h.tabla)) continue
      try {
        const [filas] = await connection.execute(
...
export async function subirBaseDatos(datos, contextoUsuario = null) {
  ...
  try {
    ...
    const resultado = await conPermisoEscrituraOffline(async () => {
      ...
      // Tablas transaccionales/generadas (financiamiento) que NUNCA deben
      // reconciliarse por borrado: un dispositivo móvil solo tiene una copia
      // PARCIAL o desactualizada de estas tablas (otros dispositivos/la web
      // también crean contratos, cuotas y pagos), así que borrar "lo que no
      // está en el JSON local" puede eliminar datos legítimos creados por
      // otros medios. Solo se hace INSERT/UPDATE (upsert), nunca DELETE.
      const TABLAS_SIN_RECONCILIACION_BORRADO = new Set([
        'fin_contratos',
        'fin_contrato_activos',
        'fin_contrato_categorias',
        'fin_cuotas',
        'fin_fiadores',
        'fin_pagos',
        'fin_pago_cuotas',
      ...
      try {
        for (const [nombre, filas] of Object.entries(datos.tablas)) {
          ...
          if (filas.length === 0) {
            if (tieneEmpresa && tieneId && !TABLAS_SIN_RECONCILIACION_BORRADO.has(nombre)) {
              try {
