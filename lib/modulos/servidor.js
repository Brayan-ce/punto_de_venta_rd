...
/**
 * Obtener el valor de permite_impresion de una empresa
 */
export async function obtenerPermisoImpresionEmpresa(empresaId) {
    ...
    try {
        ...
        const [rows] = await connection.execute(
            `SELECT permite_impresion FROM empresas WHERE id = ? LIMIT 1`,
            [empresaId]
        )
        ...
        return Boolean(rows[0].permite_impresion)

    } catch (error) {
        console.error('Error al obtener permite_impresion de empresa:', error)
...
/**
 * Cambiar permite_impresion de una empresa y cascadear a todos sus usuarios
 */
export async function togglePermisoImpresionEmpresa(empresaId, habilitado) {
    ...
    try {
        ...
        const valor = habilitado ? 1 : 0

        // Actualizar la empresa
        await connection.execute(
            `UPDATE empresas SET permite_impresion = ? WHERE id = ?`,
            [valor, empresaId]
        )

        // Cascadear a todos los usuarios de la empresa
        // Solo afecta usuarios tipo admin, vendedor (no superadmin)
        await connection.execute(
            `UPDATE usuarios SET permite_impresion = ? WHERE empresa_id = ? AND tipo IN ('admin', 'vendedor', 'financiamiento', 'sucursales')`,
            [valor, empresaId]
        )
        ...
        return {
            success: true,
            mensaje: habilitado
                ? 'Impresión habilitada para todos los usuarios de la empresa'
                : 'Impresión deshabilitada para todos los usuarios de la empresa'
        }

    } catch (error) {
        console.error('Error al toggle permite_impresion de empresa:', error)
        ...
        return {
            success: false,
            mensaje: error.message || 'Error al actualizar permiso de impresión'
        }
    }
}
